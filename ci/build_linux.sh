OPTS="-Dgraphics/qt_qpainter:BOOL=FALSE -Dgui/qml:BOOL=FALSE -DSVG2PNG:BOOL=FALSE -DSAMPLE_MAP=n -DUSE_GOOGLEPLACES:BOOL=TRUE"
if [[ "${CIRCLE_PROJECT_USERNAME}" == "navit-gps" && "${CIRCLE_BRANCH}" == "trunk" ]]; then
        ...
        mkdir bin && cd bin
        cov-build --dir cov-int cmake ${OPTS} ../
