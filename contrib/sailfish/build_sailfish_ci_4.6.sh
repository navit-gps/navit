...
#64 bit devices
sb2 -t SailfishOS-${VERSION_ID}-aarch64 -m sdk-install -R zypper --non-interactive in $(grep "^BuildRequires: " navit-sailfish4.spec | sed -e "s/BuildRequires: //")
sb2 -t SailfishOS-${VERSION_ID}-aarch64 -m sdk-build rpmbuild --define "_topdir /home/appsupport-root/rpmbuild" --define "navit_source ${SCRIPTPATH}/../.." -bb navit-sailfish4.spec
