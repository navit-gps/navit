...
Normally i have Navit installed on my smartphone and searching locations
...
-  Probably we can also sync maps by having a sync adapter which
   retrieves the needed information from the smartphone and sends them
   to the Navi
-  We can sync Notifications to the Navi to display then when wanted
-  We can use the Smartphone Mobile Data to retive traffic information
   if user wants to.
...
First one would be to write and standalone app to do the sync of
