...
+----------------------------------+----------------------------------+
| ``menu(fromosd,                  | Brings up the menu (may be used  |
| href,                            | as ``gui.menu()`` ). First       |
| replace)``                       | optional argument, fromosd, is   |
...
|                                  | :``gui.menu("#Actions",1)``.     |
+----------------------------------+----------------------------------+
| ``osd_set_attr(name,             | Sets the attribute of an OSD     |
| attr,                            | item. The OSD item must have a   |
| value)``                         | **name** attribute matching the  |
|                                  | **name** parameter in the call.  |
|                                  | **attr** is the name of the      |
|                                  | attribute, and **value** is the  |
|                                  | new value.                       |
+----------------------------------+----------------------------------+
| ``pois(position,                 | Displays POIs around given       |
| filter,                          | point, possibly filtering them   |
| isaddressfilter)``               | by text string as it would be    |
|                                  | entered in POI filtering dialog. |
|                                  | Second and third arguments may   |
|                                  | be omitted.                      |
+----------------------------------+----------------------------------+
| ``position(position,             | Presents possible operations on  |
| text, flags)``                   | a position (set as current       |
...
|                                  | the active vehicle.              |
+----------------------------------+----------------------------------+
| ``spawn(command,                 | | Launches the external program  |
| param1                           |   specified by **command**,      |
| param2)``                        |   passing it the parameters      |
...
|                                  | town.                            |
+----------------------------------+----------------------------------+
| ``write(``\ ``attribute``\ ``)`` | Writes an attribute. Used by the |
