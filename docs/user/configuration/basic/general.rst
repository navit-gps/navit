...
Coordinates can be written in different formats; see :doc:`Coordinate format </user/configuration/Coordinate_format>` for the full list.
...
Note that once Navit has started, the zoom level can be altered using :doc:`OSD </user/configuration/OSD>` or menu items.
...
Imperial units
--------------
By default, Navit uses the metric system of measurements when displaying or announcing distances, speeds etc. However, you can configure Navit to display and announce these values in imperial units. Simply add an ``imperial`` attribute to the Navit tag, and set its value to 1, as shown below:
...
The ``default_layout`` attribute of the navit tag allows to specify which layout to use as default;
