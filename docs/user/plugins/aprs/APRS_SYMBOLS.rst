...
The APRS symbols can be integrated into the plugin and installed automatically during the Navit build process. No separate installation step is required when built that way.
...
2. The symbols can then be installed during ``make install`` or when building Navit.
