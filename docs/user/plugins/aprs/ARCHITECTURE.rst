...
2. **Method 2: Symbol Lookup (Fallback)**
   ...
   Used if category lookup fails (for example, plugin not yet initialized).
...
6. **Dependencies**: APRS plugin does not require librtlsdr.
...
See ``TEST_RESULTS.rst`` for complete test documentation.
