...
- RTL-SDR dongles (fully tested and compatible):
  ...
  - Generic RTL2832U-based devices
...
See ``RTL_SDR_SETUP.rst`` for complete RTL-SDR setup instructions.
...
- **Update Latency**: < 100 ms from packet to map display (typical).
...
1. **Symbol Rendering**: Extended APRS symbol table support and theming.
...
2. **Message Types**: Currently focused primarily on position reports.
3. **Compression**: Compressed formats may need further extension depending on use cases.
...
3. The APRS plugin will be built and installed as part of the normal build.
