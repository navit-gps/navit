...
Unit tests for the APRS plugin itself are in ``navit/plugin/aprs/tests``:

- ``test_aprs_db.c`` – database operation tests
- ``test_aprs_decode.c`` – packet decoding tests

Tests that exercise the seam between ``aprs`` and ``aprs_sdr`` live in
``navit/plugin/tests_integration/aprs_sdr`` (they pull in both plugins' code):
...
.. code-block:: bash
   ...
   ./navit/plugin/tests_integration/aprs_sdr/test_aprs_sdr_integration
