...
DSP (Digital Signal Processing) tests are available in ``navit/plugin/tests_integration/aprs_sdr/test_aprs_dsp.c``. These test the Bell 202 demodulation without requiring hardware:
...
.. code-block:: bash
   ...
   cd build
   ./navit/plugin/tests_integration/aprs_sdr/test_aprs_dsp
...
End-to-end SDR integration tests are implemented in ``navit/plugin/tests_integration/aprs_sdr/test_aprs_sdr_integration.c``. They
