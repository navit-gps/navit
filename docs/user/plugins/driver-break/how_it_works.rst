...
**Code to look at:** ``energy_model_init``, ``energy_calculate_segment``, ``driver_break_compute_route_energy``, ``driver_break_check_srtm_coverage`` (``driver_break_energy.c``, ``driver_break_energy_route.c``). Kinetic analysis runs when ``use_energy_routing`` is enabled and a route is set (or when toggled on).
...
After route changes, ``driver_break_compute_route_energy`` walks the planned
route geometry, samples SRTM elevation between consecutive points, and
accumulates segment energy and an estimated fuel use. Results are shown as
Navit user messages and in the fuel configuration dialog when available.
