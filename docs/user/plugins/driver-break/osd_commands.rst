...
.. list-table::
   ...
   * - ``driver_break_configure``
     - None
     - Open Driver Break settings for the active travel mode (same as ``driver_break_open_settings``).
...
.. list-table::
   ...
   * - ``srtm_download_here``
     - None
     - Download SRTM tiles for a small bounding box around the **current vehicle position** (about 2x2 degrees). Requires libcurl.
   * - ``srtm_download_route``
     - None
     - Download SRTM tiles covering the **active route** bounding box (plus a small margin). Requires an active route and libcurl.
   * - ``srtm_download_view``
     - None
     - Download SRTM tiles for a wider box around the current position (map-view fallback, about 3x3 degrees). Requires libcurl.
   * - ``srtm_get_elevation``
     - Optional coordinate
...
Predefined regions use **fixed bounding boxes** in ``driver_break_srtm.c``. In addition, location-based commands download tiles for:

- **Current position** (``srtm_download_here``)
- **Active route corridor** (``srtm_download_route``)
- **Map view fallback** around the vehicle (``srtm_download_view``)

When **kinetic routing** is enabled and tiles are missing along the route or near the vehicle, the plugin posts a user message suggesting these download commands.
