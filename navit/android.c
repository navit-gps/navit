 ...
 */
JNIEXPORT void JNICALL Java_org_navitproject_navit_Navit_navitMain(JNIEnv *env, jobject thiz, jstring lang,
                                                                   jstring path, jstring map_path) {
    const char *langstr;
...
JNIEXPORT void JNICALL Java_org_navitproject_navit_NavitGraphics_buttonCallback(JNIEnv *env, jobject thiz, jlong id,
                                                                                jint pressed, jint button, jint x,
                                                                                jint y) {
...
JNIEXPORT jobjectArray JNICALL Java_org_navitproject_navit_NavitGraphics_getAllCountries(JNIEnv *env, jclass thiz) {
    ...
    while ((res = search_list_get_result(search_list))) {
        ...
        if (strlen(res->country->iso2) == 2) {
            ...
            current_country =
                (jobjectArray)(*env)->NewObjectArray(env, 2, (*env)->FindClass(env, "java/lang/String"), NULL);
    ...
    all_countries =
        (jobjectArray)(*env)->NewObjectArray(env, country_count, (*env)->GetObjectClass(env, current_country), NULL);
...
JNIEXPORT jstring JNICALL Java_org_navitproject_navit_NavitGraphics_getCoordForPoint(JNIEnv *env, jobject thiz, jint x,
                                                                                     jint y, jboolean absoluteCoord) {
...
JNIEXPORT jobject JNICALL Java_org_navitproject_navit_NavitCallbackHandler_callbackCmdChannel(JNIEnv *env, jclass thiz,
                                                                                              jint command) {
...
JNIEXPORT jint JNICALL Java_org_navitproject_navit_NavitCallbackHandler_callbackMessageChannel(JNIEnv *env, jclass thiz,
                                                                                               jint channel,
                                                                                               jstring str) {
    struct attr attr;
    const char *s;
...
static enum attr_type android_search_level[] = {attr_town_or_district_name, attr_street_name, attr_house_number};
...
static void android_search_idle(struct android_search_priv *search_priv) {
    ...
    if (res) {
        ...
        dbg(lvl_info, "test phrase: %d,%d, %d, level %d", search_priv->current_phrase_per_level[0],
            search_priv->current_phrase_per_level[1], search_priv->current_phrase_per_level[2], level) do {
            while (!search_priv->phrases[++search_priv->current_phrase_per_level[level]]) {
                dbg(lvl_info, "next phrase: %d,%d, %d, level %d", search_priv->current_phrase_per_level[0],
                    search_priv->current_phrase_per_level[1], search_priv->current_phrase_per_level[2],
                    level) if (level > 0) {
        ...
        while (level > 0
                   ? search_priv->current_phrase_per_level[level] == search_priv->current_phrase_per_level[level - 1]
                   : 0)
            ;
...
JNIEXPORT jlong JNICALL Java_org_navitproject_navit_NavitAddressSearchActivity_callbackStartAddressSearch(
    JNIEnv *env, jobject thiz, jint partial, jstring country, jstring str) {
    struct attr attr;
...
JNIEXPORT void JNICALL Java_org_navitproject_navit_NavitAddressSearchActivity_callbackCancelAddressSearch(
