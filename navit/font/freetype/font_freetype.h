...
struct font_freetype_methods {
     ...
     */
    struct font_freetype_font *(*font_new)(struct graphics_priv *gr, struct graphics_font_methods *meth, char *font,
                                           int size, int flags);
    void (*get_text_bbox)(struct graphics_priv *gr, struct font_freetype_font *font, char *text, int dx, int dy,
     ...
     */
    int (*get_shadow)(struct font_freetype_glyph *glyph, unsigned char *data, int stride, struct color *fg,
                      struct color *tr);
    /**
     ...
     *
     * @param g glyph to render, usually obtained from a struct font_freetype_text created via
     * font_freetype_methods.text_new()
     ...
     * @returns 0 if depth is invalid, 1 otherwise
     */
    int (*get_glyph)(struct font_freetype_glyph *glyph, unsigned char *data, int stride, struct color *fg,
                     struct color *bg, struct color *tr);
};
