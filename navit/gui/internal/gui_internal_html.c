...
static void gui_internal_set_refresh_callback(struct gui_priv *this, char *cond) {
    ...
    if (cond) {
        ...
        menu_data->refresh_callback.u.callback =
            callback_new_attr_2(callback_cast(gui_internal_refresh_callback_called), type, this, menu_data);
    ...
    case html_tag_a:
        if (html->name && len) {
            if (html->class && !strcasecmp(html->class, "clist"))
                this->html_container = gui_internal_box_new(this, gravity_left_top | orientation_vertical | flags_expand
                                                                      | flags_fill /* |flags_scrolly */);
            else
                this->html_container = gui_internal_box_new(this, gravity_center | orientation_horizontal_vertical
    ...
    case html_tag_img:
        if (len) {
            ...
            gui_internal_widget_append(
                w, gui_internal_text_new(this, _(text_stripped), gravity_left_top | orientation_vertical | flags_fill));
            html->w = w;
        }
        break;
    case html_tag_div:
...
void gui_internal_html_menu(struct gui_priv *this, const char *document, char *anchor) {
    ...
    if (this->keyboard_required) {
        ...
        if (this->keyboard)
            gui_internal_widget_append(
                this->html_container,
                gui_internal_keyboard(this, VKBD_FLAG_2 | gui_internal_keyboard_init_mode(getenv("LANG"))));
        else
