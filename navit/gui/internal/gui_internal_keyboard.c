...
gui_internal_cmd_keypress(struct gui_priv *this, struct widget *wm, void *data)
{
	...
	if (md->keyboard_mode == 66) // Greek @OLF
		gui_internal_keyboard_do(this, md->keyboard, 74);
}
...
// A list of available keyboard modes.
...
/* {"<title>", font, case_mode, umlaut_mode} */		
	/* 0*/ {"ABC", 2,  8, 24},
	/* 8*/ {"abc", 2,  0, 32},
	/*16*/ {"123", 2,  0, 24},
	/*24*/ {"ÄÖÜ", 2, 40,  0},
	...
	/*48*/ {"абв", 2, 40,  8},
	/*56*/ {"DEG", 2,  2,  2},
	/*64*/ {"ABΓ", 2, 72,  0}, /* Greek keyboard upper case  @OLF */
	/*72*/ {"abγ", 2, 64,  8}  /* Greek keyboard lower case  @OLF */
};


// Some macros that make the keyboard layout easier to visualize in
...
gui_internal_keyboard_do(struct gui_priv *this, struct widget *wkbdb, int mode)
{
	...
	wkbd->p.y=max_h*2;
	if((mode>=40&&mode<56)||(mode>=64&&mode<80)) { // Russian/Ukrainian/Belorussian/Greek layout needs more space... @OLF
		max_h=max_h*4/5;
		max_w=max_w*8/9;
		wkbd->cols=9;
	}
	...
	if (mode >= 16 && mode < 24) {
		...
		if (mode == 16) {
			...
			if(mode==64)
				...
				MODE(64); /* Greek Keyboard upper / lower case @OLF */
			MODE(40);
			MODE(48);
			wk=gui_internal_keyboard_key_data(this, wkbd, hide, 0, gui_internal_keyboard_change, wkbdb, NULL,max_w,max_h);
			wk->datai=mode+1024;
	...
	/* Greek keyboard upper case @OLF */
        if (mode >= 64 && mode < 72) {
                KEY("Α"); KEY("Β"); KEY("Γ"); KEY("Δ"); KEY("Ε"); KEY("Ζ"); KEY("Η"); KEY("Θ"); KEY("Ι");
                KEY("Κ"); KEY("Λ"); KEY("Μ"); KEY("Ν"); KEY("Ξ"); KEY("Ο"); KEY("Π"); KEY("Ρ"); KEY("Σ");
                KEY("Τ"); KEY("Υ"); KEY("Φ"); KEY("Χ"); KEY("Ψ"); KEY("Ω"); KEY("Ή"); KEY("Ά"); KEY("Ό");
                KEY("Ί"); KEY("Ώ"); KEY("Έ"); KEY("Ύ");
                gui_internal_keyboard_key(this, wkbd, space," ",max_w,max_h);
...
gui_internal_keyboard_init_mode(char *lang)
{
	...
	/*
	* Set cyrillic keyboard for countries using Cyrillic alphabet, set greek keyboard for Greece @OLF
	*/
	if (strstr(lang,"RU"))
	    ret = 40;
	if (strstr(lang,"UA"))
	...
	if (strstr(lang,"MN"))
	    ret = 40;
	if (strstr(lang,"GR")) /* Greek keyboard @OLF */
	    ret = 64;
	g_free(lang);
	return ret;
}
