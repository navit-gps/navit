 ...
 * dimensions will be set to match those of the display
 *
 ...
 * @param w A widget corresponding to the menu to redraw (this widget should be an html menu, thus it should have a
 * href)
 *
 ...
 * @param w A widget corresponding to the last menu to keep (all subsequent menus in the list will be destroyed). NULL
 * if all menus should be destroyed.
...
static void gui_internal_prune_menu_do(struct gui_priv *this, struct widget *w, int render) {
    ...
    while ((l = g_list_last(this->root.children))) {
        wd = l->data;
        if (wd == w) { /* This is the widget we want to bring back to display (all subsequent widgets will be destroyed
                          in the loop) */
            ...
            if (redisplay) {
                ...
                /* Make sure the new menu we are going to display spawns the whole display (in case there was a resize
                 * while a submenu was being displayed) */
                w->w = this->root.w;
                w->h = this->root.h;
                /* If the foremost widget is a HTML menu, reload & redraw it from its href using
...
/**
 * @brief Destroy (discard) all menu screens that have been placed after widget @p w and redraw the display, showing
 * widget @w
 *
 * @param this The internal GUI instance
 * @param w A widget corresponding to the last menu to keep, and to draw (all subsequent menus in the list will be
 * destroyed). NULL if all menus should be destroyed.
 */
 ...
 *
 * Menus are stored in a main box (topbox) that spawn the entire screen, thus, once the topbox has been resized, it will
 * in turn resize its children, effectively resizing the whole menu display size
 *
 * Note: once this function has been run, widget @p w and its children have been resized, a call to
 * gui_internal_box_render() needs to be done by the caller
 *
 * @param this The internal GUI instance
 ...
 *
 * @note The whole sequence of menus is kept in this->root.children (when going back one page, we just move to the
 * previous child in the list) Thus, only the last child of this->root.children is actually displayed
 *
 * @param this The internal GUI instance
 */
...
struct widget *gui_internal_top_bar(struct gui_priv *this) {
    ...
    */
    ...
    if (!(this->flags & 2)) {
        wm = gui_internal_button_new_with_callback(this, NULL, image_new_s(this, "gui_map"),
                                                   gravity_center | orientation_vertical, gui_internal_cmd_return,
                                                   NULL);
    ...
    if (!(this->flags & 4)) {
        wh = gui_internal_button_new_with_callback(this, NULL, image_new_s(this, "gui_home"),
                                                   gravity_center | orientation_vertical, gui_internal_cmd_main_menu,
                                                   NULL);
    ...
    while (l) {
        if (g_list_previous(l) || !g_list_next(l)) {
            ...
            if (g_list_next(l))
                ...
                use_sep = 0;
            dbg(lvl_debug, "%d (%s) + %d + %d + %d > %d", wcn->w, wc->text, width_used, w->spx, use_sep ? sep_len : 0,
                width);
