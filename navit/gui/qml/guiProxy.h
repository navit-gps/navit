...
class NGQProxyGui : public NGQProxy {
  ...
  public:
    ...
    void processCommand(QString function) {
        ...
        // commandJS(this->object->guiWidget->rootContext(),QString(),qobject_cast<QObject*>(this->object->guiWidget->rootObject()));
  ...
  public slots:
    ...
    void switchToMenu(struct point *p) {
        if (!this->object->lazy) {
            ...
            this->object->guiWidget->setSource(
                QUrl::fromLocalFile(QString(this->object->source) + "/" + this->object->skin + "/main.qml"));
        }
  ...
  protected:
    int getAttrFunc(enum attr_type type, struct attr *attr, struct attr_iter *iter) {
        return gui_get_attr(this->object->gui, type, attr, iter);
    }
    int setAttrFunc(struct attr *attr) {
        return gui_set_attr(this->object->gui, attr);
    }
