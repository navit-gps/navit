        ...
        if (item_is_point(*item) && graphics_displayitem_get_displayed(di)
            ...
            if (item_attr_get(itemo, attr_data, &attr)) {
                ...
                if (navit_get_attr(this_->nav, attr_callback_list, &cb, NULL))
                    callback_list_call_attr_4(cb.u.callback_list, attr_command, "dbus_send_signal", attr_list, NULL,
                                              &valid);
            }
            map_rect_destroy(mr);
        }
...
struct gui_methods gui_qml_methods = {
    NULL, NULL, gui_qml_set_graphics, NULL, NULL, NULL, NULL, gui_qml_get_attr, NULL, gui_qml_set_attr,
};
