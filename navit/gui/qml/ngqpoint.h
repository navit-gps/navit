...
class NGQPoint : public QObject {
  ...
  public:
    ...
    NGQPoint(struct gui_priv *this_, struct coord *c, QString name, NGQPointTypes type = Bookmark,
             QObject *parent = NULL)
        : QObject(parent) {
        this->object = this_;
        this->item.map = 0;
        ...
        if (this->type != Bookmark and this->item.map) {
            ...
            if (item) {
                while (item_attr_get(item, attr_any, &attr)) {
                    entries.appendChild(
                        this->_fieldValueHelper(retDoc, QString::fromLocal8Bit(attr_to_name(attr.type)),
                                                QString::fromLocal8Bit(attr_to_text(&attr, this->item.map, 1))));
                }
            }
            map_rect_destroy(mr);
        }
        ...
        while ((m = mapset_next(h, 1))) {
            ...
            if (mr) {
                while ((item = map_rect_get_item(mr))) {
                    struct coord c;
                    if (item_coord_get_pro(item, &c, 1, this->c.pro) && coord_rect_contains(&sel->u.c_rect, &c)
                        && (idist = transform_distance(this->c.pro, &center, &c)) < dist && item->type < type_line) {
                        char *label;
                        QString rs;
  ...
  protected:
    ...
    QString _coordString() {
        ...
        return QString(QString::fromLocal8Bit("%1°%2'%3\" %4%5%6°%7'%8\" %9"))
            .arg(lat_deg)
            .arg(lat_min)
            .arg(lat_sec)
            .arg(latc)
