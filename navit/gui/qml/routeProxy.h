...
class NGQProxyRoute : public NGQProxy {
  ...
  public slots:
    void addDestination() {
        ...
        struct pcoord *coords = (struct pcoord *)malloc(
            sizeof(struct pcoord) * (destinations.size() + 1));  // Additional destination included
  ...
  protected:
    int getAttrFunc(enum attr_type type, struct attr *attr, struct attr_iter *iter) {
        return route_get_attr(navit_get_route(this->object->nav), type, attr, iter);
    }
    int setAttrFunc(struct attr *attr) {
        return route_set_attr(navit_get_route(this->object->nav), attr);
    }
    struct attr_iter *getIterFunc() {
        return route_attr_iter_new();
    };
    void dropIterFunc(struct attr_iter *iter) {
        route_attr_iter_destroy(iter);
    };
