...
static void CreateToolBar(HWND hwnd) {
    ...
    hTool = CreateWindowEx(0, TOOLBARCLASSNAME, NULL, WS_CHILD | WS_VISIBLE, 0, 0, 0, 0, hwnd, (HMENU)ID_CHILD_TOOLBAR,
...
static void window_layout(HWND hwnd) {
    ...
    if (hChild) {
        MoveWindow(hChild, rcClient.left, rcClient.top, rcClient.right - rcClient.left, rcClient.bottom - rcClient.top,
                   TRUE);
...
static void toggle_fullscreen(HWND mWnd) {
    if (SHFullScreenPtr) {
        if (!ce_fullscreen) {
            (*SHFullScreenPtr)(mWnd, SHFS_HIDETASKBAR | SHFS_HIDESTARTICON | SHFS_HIDESIPBUTTON);
        } else {
    ...
    case WM_COMMAND: {
        ...
        if (popup_menu_array) {
            struct menu_priv *priv =
                (struct menu_priv *)g_array_index(popup_menu_array, gint, LOWORD(wParam) - POPUP_MENU_OFFSET);
...
static HANDLE CreateWin32Window(void) {
    ...
    hwnd = CreateWindowEx(WS_EX_CLIENTEDGE, g_szClassName, TEXT("Navit"),
#ifdef HAVE_API_WIN32_CE
                          WS_SYSMENU | WS_CLIPCHILDREN, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
#else
