...
static void notify_size(struct datawindow_priv *datawindow, int width, int height) {
    if (datawindow) {
        MoveWindow(datawindow->hwndLabel, 0, 0,                         // starting x- and y-coordinates
                   width,                                               // width of client area
                   20,                                                  // height of client area
                   TRUE);                                               // repaint window
        MoveWindow(datawindow->hwndEdit, 0, 20,                         // starting x- and y-coordinates
                   width,                                               // width of client area
                   20,                                                  // height of client area
                   TRUE);                                               // repaint window
        MoveWindow(datawindow->hwndList, 0, 40,                         // starting x- and y-coordinates
                   width,                                               // width of client area
                   height - 60,                                         // height of client area
                   TRUE);                                               // repaint window
        MoveWindow(datawindow->hwndButtonPrev, 0, height - 20,          // starting x- and y-coordinates
                   width / 2,                                           // width of client area
                   20,                                                  // height of client area
                   TRUE);                                               // repaint window
        MoveWindow(datawindow->hwndButtonNext, width / 2, height - 20,  // starting x- and y-coordinates
                   width / 2,                                           // width of client area
                   20,                                                  // height of client area
                   TRUE);                                               // repaint window
...
HANDLE create_destination_window(struct navit *nav) {
    ...
    this_->hwnd = CreateWindowEx(WS_EX_CLIENTEDGE, g_szDestinationClassName, TEXT("Destination Input"),
#if defined(__CEGCC__)
                                 WS_SYSMENU | WS_CLIPCHILDREN, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                                 CW_USEDEFAULT,
#else
                                 WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN, CW_USEDEFAULT, CW_USEDEFAULT, 640, 480,
#endif
                                 NULL, NULL, NULL, NULL);
    ...
    this_->hwndEdit =
        CreateWindow(WC_EDIT,                                      // predefined class
    ...
    this_->hwndList = CreateWindow(WC_LISTVIEW,  // predefined class
                                   ...
                                   WS_CHILD | WS_VISIBLE | ES_LEFT | WS_BORDER
                                       | LVS_REPORT,  //| WS_VSCROLL | ES_MULTILINE | ES_AUTOVSCROLL
                                   0, 0, 0, 0,        // set size in WM_SIZE message
                                   this_->hwnd,       // parent window
                                   NULL,              //(HMENU) ID_EDITCHILD,   // edit control ID
                                   (HINSTANCE)GetWindowLong(this_->hwnd, GWL_HINSTANCE),
