...
#define rects_overlap(x11, y11, x12, y12, x21, y21, x22, y22)                                                          \
...
void quadtree_find_rect_items(struct quadtree_node *this_, double dXMin, double dXMax, double dYMin, double dYMax,
    ...
    if (this_->is_leaf) {
        ...
        for (i = 0; i < 4; ++i) {
            if (nodes[i]) {
                // limit flooding
                if (nodes[i]->xmax < dXMin || dXMax < nodes[i]->xmin || nodes[i]->ymax < dYMin
                    || dYMax < nodes[i]->ymin) {
                    continue;
                }
...
struct quadtree_item *quadtree_find_nearest_flood(struct quadtree_node *this_, struct quadtree_item *item,
    ...
    if (this_->is_leaf) {
        ...
        for (i = 0; i < this_->node_num; ++i) {
            double curr_dist_sq =
                dist_sq(item->longitude, item->latitude, this_->items[i]->longitude, this_->items[i]->latitude);
        ...
        for (i = 0; i < 4; ++i) {
            if (nodes[i] && nodes[i] != toSkip) {
                ...
                if (dist_sq(nodes[i]->xmin, nodes[i]->ymin, item->longitude, item->latitude) < current_max
...
struct quadtree_item *quadtree_find_item(struct quadtree_node *this_, struct quadtree_item *item) {
    ...
    if (this_->is_leaf) {
        int i;
        for (i = 0; i < this_->node_num; ++i) {
            // TODO equality check may not be correct on float values! maybe it can be replaced by range check with some
            // tolerance
        ...
        if (this_->aa && this_->aa->xmin <= item->longitude && item->longitude < this_->aa->xmax
            && this_->aa->ymin <= item->latitude && item->latitude < this_->aa->ymax) {
            res = quadtree_find_item(this_->aa, item);
        } else if (this_->ab && this_->ab->xmin <= item->longitude && item->longitude < this_->ab->xmax
                   && this_->ab->ymin <= item->latitude && item->latitude < this_->ab->ymax) {
            res = quadtree_find_item(this_->ab, item);
        } else if (this_->ba && this_->ba->xmin <= item->longitude && item->longitude < this_->ba->xmax
                   && this_->ba->ymin <= item->latitude && item->latitude < this_->ba->ymax) {
            res = quadtree_find_item(this_->ba, item);
        } else if (this_->bb && this_->bb->xmin <= item->longitude && item->longitude < this_->bb->xmax
...
/*
 * tries to find closest item, first it descend into the quadtree as much as possible, then if no point is found go up n
 * levels and flood
 */
...
void quadtree_add(struct quadtree_node *this_, struct quadtree_item *item, struct quadtree_iter *iter) {
    if (this_->is_leaf) {
        ...
        if (QUADTREE_NODE_CAPACITY - 1 == this_->node_num) {
            ...
            if (bSame) {
                // FIXME: memleak and items thrown away if more than QUADTREE_NODE_CAPACITY-1 items with same
                // coordinates added.
                dbg(lvl_error,
                    "Unable to add another item with same coordinates. Throwing item to the ground. Will leak %p.",
                    item);
                return;
            }
        ...
        if (this_->xmin <= item->longitude && item->longitude < this_->xmin + (this_->xmax - this_->xmin) / 2.0
            && this_->ymin <= item->latitude && item->latitude < this_->ymin + (this_->ymax - this_->ymin) / 2.0) {
            if (!this_->aa) {
                this_->aa = quadtree_node_new(this_, this_->xmin, this_->xmin + (this_->xmax - this_->xmin) / 2.0,
                                              this_->ymin, this_->ymin + (this_->ymax - this_->ymin) / 2.0);
            }
            quadtree_add(this_->aa, item, iter);
        } else if (this_->xmin + (this_->xmax - this_->xmin) / 2.0 <= item->longitude && item->longitude < this_->xmax
                   && this_->ymin <= item->latitude
                   && item->latitude < this_->ymin + (this_->ymax - this_->ymin) / 2.0) {
            if (!this_->ab) {
                this_->ab = quadtree_node_new(this_, this_->xmin + (this_->xmax - this_->xmin) / 2.0, this_->xmax,
                                              this_->ymin, this_->ymin + (this_->ymax - this_->ymin) / 2.0);
            }
            quadtree_add(this_->ab, item, iter);
        } else if (this_->xmin <= item->longitude && item->longitude < this_->xmin + (this_->xmax - this_->xmin) / 2.0
                   && this_->ymin + (this_->ymax - this_->ymin) / 2.0 <= item->latitude
                   && item->latitude < this_->ymax) {
            if (!this_->ba) {
...
void quadtree_split(struct quadtree_node *this_) {
    ...
    this_->is_leaf = 0;
    for (i = 0; i < this_->node_num; ++i) {
        if (this_->xmin <= this_->items[i]->longitude
            && this_->items[i]->longitude < this_->xmin + (this_->xmax - this_->xmin) / 2.0
            && this_->ymin <= this_->items[i]->latitude
            && this_->items[i]->latitude < this_->ymin + (this_->ymax - this_->ymin) / 2.0) {
            if (!this_->aa) {
                this_->aa = quadtree_node_new(this_, this_->xmin, this_->xmin + (this_->xmax - this_->xmin) / 2.0,
                                              this_->ymin, this_->ymin + (this_->ymax - this_->ymin) / 2.0);
            }
            quadtree_add(this_->aa, this_->items[i], NULL);
        } else if (this_->xmin + (this_->xmax - this_->xmin) / 2.0 <= this_->items[i]->longitude
                   && this_->items[i]->longitude < this_->xmax && this_->ymin <= this_->items[i]->latitude
                   && this_->items[i]->latitude < this_->ymin + (this_->ymax - this_->ymin) / 2.0) {
            if (!this_->ab) {
                this_->ab = quadtree_node_new(this_, this_->xmin + (this_->xmax - this_->xmin) / 2.0, this_->xmax,
                                              this_->ymin, this_->ymin + (this_->ymax - this_->ymin) / 2.0);
        ...
        } else if (this_->xmin <= this_->items[i]->longitude
                   && this_->items[i]->longitude < this_->xmin + (this_->xmax - this_->xmin) / 2.0
                   && this_->ymin + (this_->ymax - this_->ymin) / 2.0 <= this_->items[i]->latitude
                   && this_->items[i]->latitude < this_->ymax) {
            if (!this_->ba) {
                this_->ba = quadtree_node_new(this_, this_->xmin, this_->xmin + (this_->xmax - this_->xmin) / 2.0,
                                              this_->ymin + (this_->ymax - this_->ymin) / 2.0, this_->ymax);
            }
            quadtree_add(this_->ba, this_->items[i], NULL);
                   ...
                   && this_->items[i]->longitude < this_->xmax
                   && this_->ymin + (this_->ymax - this_->ymin) / 2.0 <= this_->items[i]->latitude
                   && this_->items[i]->latitude < this_->ymax) {
            if (!this_->bb) {
                this_->bb = quadtree_node_new(this_, this_->xmin + (this_->xmax - this_->xmin) / 2.0, this_->xmax,
 ...
 * @param item_free function to be called to free the qitem, first parameter would be context, second parameter - actual
 *qitem pointer to free if this parameter is NULL, nothing would be called to free qitem pointer (possible massive
 *memleak unless you store each qitem pointer in some alternate storage)
...
struct quadtree_iter *quadtree_query(struct quadtree_node *this_, double dXMin, double dXMax, double dYMin,
    ...
    ret->ymin = dYMin;
    ret->ymax = dYMax;
    dbg(lvl_debug, "%f %f %f %f", dXMin, dXMax, dYMin, dYMax) ret->item_free = item_free;
    ...
    this_->ref_count++;
    dbg(lvl_debug, "Query %p ", this_) return ret;
}
    ...
    while (iter->iter_nodes) {
        ...
        if (iter_node->is_leaf) {
            ...
            for (subnode = NULL; !subnode && iter_node->subnode < 4; iter_node->subnode++) {
                i = iter_node->subnode;
                if (!nodes[i]
                    || !rects_overlap(nodes[i]->xmin, nodes[i]->ymin, nodes[i]->xmax, nodes[i]->ymax, iter->xmin,
                                      iter->ymin, iter->xmax, iter->ymax))
                    continue;
