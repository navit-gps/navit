...
int map_collect_data_osm_db(char *dbstr, struct maptool_osm *osm) {
    ...
    for (;;) {
        ...
        sprintf(query, "select node_id,k,v from node_tags where node_id >= %ld and node_id <= %ld order by node_id",
                min, max);
    ...
    for (;;) {
        ...
        sprintf(
            query,
            "select way_id,node_id from way_nodes where way_id >= %ld and way_id <= %ld order by way_id,sequence_id",
            min, max);
        ...
        sprintf(query, "select way_id,k,v from way_tags where way_id >= %ld and way_id <= %ld order by way_id", min,
                max);
        tag = PQexec(conn, query);
        if (!tag) {
    ...
    for (;;) {
        ...
        sprintf(query,
                "select relation_id,k,v from relation_tags where relation_id >= %ld and relation_id <= %ld order by "
                "relation_id",
                min, max);
        tag = PQexec(conn, query);
        if (!tag) {
        ...
        sprintf(query,
                "select relation_id, member_id, member_type, member_role from relation_members where relation_id >= "
                "%ld and relation_id <= %ld order by relation_id, sequence_id",
                min, max);
