...
struct navit {
    ...
    int ready; /**< Flags for Navit ready state.
                    1: Init has been done
                    2: resize has been done to set window size from grafics plugin
                    When 1&2 are combined navit is ready which has value 3
                    */
    struct window *win;
...
void navit_set_destination(struct navit *this_, struct pcoord *c, const char *description, int async) {
     ...
     * @returns nothing
     */
    void navit_set_destinations(struct navit * this_, struct pcoord * c, int count, const char *description,
                                int async) {
        char *destination_file;
        if (c && count) {
    ...
    void navit_window_roadbook_new(struct navit * this_) {
        ...
        this_->roadbook_window = gui_datawindow_new(
            this_->gui, _("Roadbook"), NULL, callback_new_1(callback_cast(navit_window_roadbook_destroy), this_));
    ...
    void navit_set_center_cursor(struct navit * this_, int autozoom, int keep_orientation) {
        ...
        if (vehicle_get_attr(nv->vehicle, attr_position_valid, &attr, NULL)
    ...
    static int navit_set_attr_do(struct navit * this_, struct attr * attr, int init) {
        ...
        case attr_pitch:
            ...
            transform_set_pitch(this_->trans,
                                round(this_->pitch * sqrt(240 * 320)
    ...
    /**
     * Toggle the cursor update : refresh the map each time the cursor has moved (instead of only when it reaches a
     * border)
     *
     * @param this_ The navit instance
    ...
    static int navit_set_vehicleprofile(struct navit * this_, struct vehicleprofile * vp) {
        ...
        if (this_->vehicleprofile == vp) {
            dbg(lvl_debug, "vehicleprofile is already set to: '%s'", name);
        } else {
    ...
    static int navit_add_vehicle(struct navit * this_, struct vehicle * v) {
        ...
        nv->callback.u.callback = callback_new_attr_3(callback_cast(navit_vehicle_update_status),
                                                      attr_position_fix_type, this_, nv, attr_position_fix_type);
        vehicle_add_attr(nv->vehicle, &nv->callback);
        nv->callback.u.callback = callback_new_attr_3(callback_cast(navit_vehicle_update_status),
                                                      attr_position_sats_used, this_, nv, attr_position_sats_used);
        vehicle_add_attr(nv->vehicle, &nv->callback);
        nv->callback.u.callback = callback_new_attr_3(callback_cast(navit_vehicle_update_status), attr_position_hdop,
                                                      this_, nv, attr_position_hdop);
        vehicle_add_attr(nv->vehicle, &nv->callback);
    ...
    void navit_layout_switch(struct navit * n) {
        ...
        if (l->dayname || l->nightname) {
            ...
            if (__sunriset__(year, month, day, geo_attr.u.coord_geo->lng, geo_attr.u.coord_geo->lat, n->sunrise_degrees,
                             1, &trise, &tset)
                != 0) {
