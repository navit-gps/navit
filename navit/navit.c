 ...
 * using navit_say async
 */
void *navit_say_previous_async(void *args) {
    dbg(lvl_debug, "enter");
    // TODO: Audio on separate thread for previous destination
    thread_args_t *arguments = (thread_args_t *)args;
    if (0) {
        dbg(lvl_debug, "this_: %s", arguments->this_);
        dbg(lvl_debug, "previous destination_announcement: '%s'", arguments->destination_announcement);
        // this line below causes crash because args has no value
        navit_say(arguments->this_, g_strdup(arguments->destination_announcement));
    } else {
        dbg(lvl_debug, "the variable 'args' has no value, so no text can be spoken");
        dbg(lvl_info, "To be implemented: Speak a previous destination");
    }
    return NULL;
}
...
static void navit_add_former_destinations_from_file(struct navit *this_) {
    ...
    if (valid && count > 0) {
        ...
        if (this_) {
            // TODO: Audio on separate thread for previous destination
            //  'args' variable is not passed to 'navit_say_previous_async' function
            dbg(lvl_debug, "previous this->destination_description: %s", this_->destination_description);
            args.this_ = this_;
            args.destination_announcement =
                g_strdup_printf("%s %s", _("destination set to"), this_->destination_description);
            dbg(lvl_debug, "previous args.this_: '%s'", args.this_);
            dbg(lvl_debug, "previous args.destination_announcement: '%s'", args.destination_announcement);
            dbg(lvl_debug, "creating thread for audio for previous destination");
            ret = pthread_create(&tid, NULL, navit_say_previous_async, (void *)&args);
            dbg(lvl_debug, "after previous destination pthread_create ret = %i", ret);
            if (ret == 0) {
                pthread_detach(tid);
            } else {
                dbg(lvl_warning, "Could not announce destination");
                dbg(lvl_debug, "Could not create thread to play audio: ret = %i", ret);
            }
        } else {
            dbg(lvl_debug, "No navit object, so can't announce destination");
        }  // this_
        g_free(destination_announcement);
        g_free(pc);
    }
