    ...
    case attr_position_time_iso8601:
        if (format) {
            if (strcmp(format, "iso8601")) {
                ...
                if ((sscanf(format, "%*c%2d:%2d;", &(text_tm.tm_hour), &(text_tm.tm_min)) == 2)
                    && (strchr("+-", format[0]))) {
                    if (strchr("-", format[0])) {
...
static void osd_text_get_value_and_draw(struct osd_priv_common *opc, struct navit *navit) {
    ...
    int do_draw = opc->osd_item.do_draw;
    struct attr attr, vehicle_attr, maxspeed_attr, imperial_attr, destination_description_attr, destination_town_attr,
        destination_street_attr, destination_house_number_attr;
    struct navigation *nav = NULL;
    struct tracking *tracking = NULL;
...
static void osd_parse_and_set_label_text(struct osd_priv_common *opc, struct navit *nav) {
    ...
    while ((start = strstr(text_to_parse, "${"))) {
        ...
        *end++ = '\0';
        ...
        // Callback
        navit_add_callback(
            nav, callback_new_attr_1(callback_cast(osd_text_get_value_and_draw), attr_position_coord_geo, opc));
