...
See [RTL_SDR_SETUP.md](RTL_SDR_SETUP.md) for detailed hardware setup instructions.
...
See [RTL_SDR_SETUP.md](RTL_SDR_SETUP.md) for detailed hardware setup and driver installation instructions.
...
```c
#include "navit/plugin/aprs/aprs.h"

// In your packet source handler:
unsigned char packet_data[256];
int packet_length = receive_aprs_packet(packet_data, sizeof(packet_data));

// Get the map instance (from Navit's map system)
struct map *aprs_map = get_aprs_map_instance();
struct map_priv *priv = map_get_priv(aprs_map);

aprs_process_packet(priv, packet_data, packet_length);
```
...
|-----------|------------------|
| **144.390 MHz** | North America (USA, Canada), Colombia, Chile, Indonesia, Malaysia, Singapore, Thailand |
...
The APRS plugin provides a comprehensive menu interface accessible from Navit's popup menu:

### Accessing the Menu

1. Open Navit's popup menu (right-click or menu button)
2. Select "APRS Settings"
