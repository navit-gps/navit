...
struct map_priv {
    ...
    /* Serializes access to the shared map state (db, items, station_hash).
     * aprs_process_packet() and aprs_nmea_callback() run on the SDR/NMEA reader
     * threads while the event-loop timers and the map render path run on the
     * main thread; this lock prevents them from racing. */
    GMutex lock;
};
...
static void aprs_update_items(struct map_priv *priv) {
    ...
    /* Hold the lock for the whole rebuild so reader threads and the render path
     * never observe a half-rebuilt items list or station_hash. */
    g_mutex_lock(&priv->lock);
    ...
    /* Release before notifying: the callback can re-enter the map (e.g. a
     * redraw acquiring the same lock via aprs_map_rect_new). */
    g_mutex_unlock(&priv->lock);
...
static void aprs_map_destroy(struct map_priv *priv) {
    ...
    /* Stop the reader thread and event sources first so nothing can mutate the
     * map state (or take priv->lock) while it is being torn down. */
    if (priv->expire_timeout) {
        event_remove_timeout(priv->expire_timeout);
        priv->expire_timeout = NULL;
    }
    if (priv->update_timeout) {
        event_remove_timeout(priv->update_timeout);
        priv->update_timeout = NULL;
    }
...
static struct map_rect_priv *aprs_map_rect_new(struct map_priv *priv, struct map_selection *sel) {
    ...
    /* Hold the lock for the lifetime of the rect (until aprs_map_rect_destroy)
     * so the items list cannot be rebuilt by a reader thread mid-iteration. */
    g_mutex_lock(&priv->lock);
    mr->mpriv = priv;
...
/* Invoked both from the main thread and from the SDR read/DSP thread (via
 * aprs_sdr_frame_delivery_callback). The shared db is mutated under priv->lock,
 * and aprs_update_items() takes the same lock while rebuilding the map state. */
static int aprs_process_packet(struct map_priv *priv, const unsigned char *data, int length) {
