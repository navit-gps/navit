...
/* NMEA serial input relies on the POSIX serial (termios), threading (pthread)
 * and file (unistd/fcntl) APIs. These are unavailable on targets such as
 * Windows/WinCE, so the implementation is guarded and replaced with stubs there
 * to keep the plugin linkable on every platform. */
#include "aprs_nmea.h"
#include "aprs_db.h"
#include "config.h"
#include "debug.h"
...
struct aprs_nmea {
    ...
    int fd;
    /* Written by the main thread (start/stop), read by the read thread.
     * volatile prevents the compiler from caching the value in a register so
     * the thread observes a stop request. */
 ...
 * In termios, PARENB enables parity and PARODD selects odd parity:
 *   even parity = PARENB (PARODD cleared)
...
/* POSIX serial/threading APIs are unavailable on this target. Provide stubs so
 * the plugin still links; NMEA serial input is simply not offered. */
...
struct aprs_nmea *aprs_nmea_new(const struct aprs_nmea_config *config) {
    (void)config;
    dbg(lvl_error, "NMEA serial input is not supported on this platform");
    return NULL;
}
