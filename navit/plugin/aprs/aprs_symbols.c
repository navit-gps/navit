...
/* Base directory the APRS symbol icons are installed to, expressed relative to
 * Navit's runtime share directory. CMakeLists installs the icons here and
 * defines APRS_SYMBOLS_BASE_PATH so lookup and install stay in sync; the
 * fallback mirrors that install location. The leading $NAVIT_SHAREDIR is
 * expanded by graphics_icon_path() when the icon is loaded. */
#ifndef APRS_SYMBOLS_BASE_PATH
#    define APRS_SYMBOLS_BASE_PATH "$NAVIT_SHAREDIR/aprs-symbols/48x48"
#endif
...
char *aprs_symbol_get_icon(char symbol_table, char symbol_code) {
    /* Lookups use the static symbol arrays via find_symbol_entry(); the only
     * per-init state needed here is icon_base_path. The unused symbol_lookup
     * hash table must not gate this path or it would always return NULL. */
    if (!icon_base_path) {
    ...
    /* Primary ('/') and alternate ('\\') tables are installed in separate
     * subdirectories. Build a path that graphics_icon_path() resolves against
     * $NAVIT_SHAREDIR at load time. */
    const char *subdir = (symbol_table == '\\') ? "alternate" : "primary";
    char *icon_path = g_build_filename(icon_base_path, subdir, entry->filename, NULL);
    return icon_path;
}
...
int aprs_symbol_init(const char *symbol_index_path, const char *icon_base) {
    ...
    /* Free any previous base path so repeated init calls do not leak. */
    ...
    if (icon_base) {
        ...
        /* Resolved against $NAVIT_SHAREDIR by graphics_icon_path(); matches the
         * CMake install location. The primary/alternate subdir is appended per
         * symbol in aprs_symbol_get_icon(). */
        icon_base_path = g_strdup(APRS_SYMBOLS_BASE_PATH);
    }
