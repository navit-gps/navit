...
static int aprs_sdr_discover_aprs_plugin(struct aprs_sdr_priv *priv) {
    ...
    if (map_priv_ptr) {
        ...
        if (aprs_module) {
            ...
            /* The resolved symbol stays valid because Navit keeps the APRS
             * plugin loaded; release our extra reference to avoid leaking the
             * module handle. */
...
static void *aprs_sdr_map_new(struct map_methods *meth, struct attr **attrs, struct callback_list *cbl) {
    ...
    *meth = aprs_sdr_map_meth;
    ...
    priv->running = 1;
    priv->initialized = 1;
    /* The APRS channel frequency is the RTL-SDR center minus the IF offset;
     * aprs_freq_mhz itself is local to aprs_sdr_fill_config_from_attrs and is
     * not in scope here. */
    dbg(lvl_info, "APRS SDR plugin started: APRS %.3f MHz, RTL-SDR center %.3f MHz, RF sample rate %d Hz",
        hw_config.frequency_mhz - (APRS_SDR_IF_OFFSET_HZ / 1000000.0), hw_config.frequency_mhz, hw_config.sample_rate);
    return priv;
}
