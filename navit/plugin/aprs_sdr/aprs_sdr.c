...
static void *aprs_sdr_map_new(struct map_methods *meth, struct attr **attrs, struct callback_list *cbl) {
    ...
    *meth = aprs_sdr_map_meth;
    ...
    freq_attr = attr_search(attrs, attr_frequency);
    if (freq_attr && freq_attr->u.numd) {
        hw_config.frequency_mhz = *freq_attr->u.numd;
    }
    
    gain_attr = attr_search(attrs, attr_gain);
    if (gain_attr) {
        hw_config.gain = gain_attr->u.num;
    }
    
    ppm_attr = attr_search(attrs, attr_ppm);
    if (ppm_attr) {
        hw_config.ppm_correction = ppm_attr->u.num;
    }
    
    device_attr = attr_search(attrs, attr_device);
    if (device_attr && device_attr->u.str) {
        if (strstr(device_attr->u.str, "blog_v3")) {
            hw_config.device_type = APRS_SDR_BLOG_V3;
        } else if (strstr(device_attr->u.str, "v4_r828d")) {
            hw_config.device_type = APRS_SDR_V4_R828D;
        } else if (strstr(device_attr->u.str, "nooelec")) {
            hw_config.device_type = APRS_SDR_NOOELEC;
        } else {
            hw_config.device_type = APRS_SDR_GENERIC;
        }
    }
    ...
    if (!aprs_sdr_discover_aprs_plugin(priv)) {
        ...
        /* Continue anyway - user might load APRS plugin later */
    }
...
static int aprs_sdr_map_set_attr(void *priv, struct attr *attr) {
    ...
    case attr_gain:
        if (sdr_priv->hw) {
            int gain = attr->u.num;
            if (aprs_sdr_hw_set_gain(sdr_priv->hw, gain)) {
                dbg(lvl_info, "SDR gain set to %d", gain);
                return 1;
            }
        }
        return 0;
    case attr_ppm:
        if (sdr_priv->hw) {
            int ppm = attr->u.num;
            if (aprs_sdr_hw_set_ppm(sdr_priv->hw, ppm)) {
                dbg(lvl_info, "SDR PPM correction set to %d", ppm);
                return 1;
            }
        }
        return 0;
