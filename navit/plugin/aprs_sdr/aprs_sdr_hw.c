...
struct aprs_sdr_hw {
    ...
    pthread_t thread;
    /* running/stop_requested are written by the main thread and read by the
     * read thread; volatile prevents the compiler caching them in a register
     * so the thread observes a stop request. */
    volatile int running;
    volatile int stop_requested;
};

/* RTL-SDR async sample buffer size (bytes). Must be a multiple of 512. */
#    define APRS_SDR_HW_BUF_LEN 16384

/* libusb async callback: invoked from within rtlsdr_read_async() on the read
 * thread for each received buffer. Forwarding to the DSP callback is synchronous,
 * so the library-owned buffer remains valid for the duration of this call. */
static void aprs_sdr_hw_async_cb(unsigned char *buf, uint32_t len, void *ctx) {
    struct aprs_sdr_hw *hw = (struct aprs_sdr_hw *)ctx;

    if (hw->stop_requested) {
        /* Ensure a stop that arrives mid-stream unwinds rtlsdr_read_async()
         * promptly instead of waiting for the device to go idle. */
        rtlsdr_cancel_async(hw->dev);
        return;
    }

    if (len > 0 && hw->callback) {
        hw->callback(buf, (int)len, hw->callback_user_data);
    }
}
...
static void *aprs_sdr_hw_read_thread(void *arg) {
    ...
    /* Blocks until rtlsdr_cancel_async() is called (from aprs_sdr_hw_stop() or
     * the async callback). buf_num=0 selects the library default buffer count. */
    result = rtlsdr_read_async(hw->dev, aprs_sdr_hw_async_cb, hw, 0, APRS_SDR_HW_BUF_LEN);
    if (result < 0) {
        dbg(lvl_error, "RTL-SDR async read ended with error: %d", result);
    }
...
int aprs_sdr_hw_stop(struct aprs_sdr_hw *hw) {
    ...
    hw->stop_requested = 1;
    /* rtlsdr_cancel_async() unblocks the rtlsdr_read_async() loop running in the
     * read thread, so pthread_join() below cannot block indefinitely. */
    rtlsdr_cancel_async(hw->dev);
