...
struct aprs_sdr_hw *aprs_sdr_hw_new(const struct aprs_sdr_hw_config *config) {
    ...
    enum aprs_sdr_device_type detected_type = aprs_sdr_hw_detect_type(dev);
    if (hw->config.device_type == APRS_SDR_UNKNOWN) {
        hw->config.device_type = detected_type;
    }
    ...
    if (hw->config.gain >= 0) {
        ...
        if (result < 0) {
            ...
            result = rtlsdr_set_tuner_gain(dev, hw->config.gain * 10);
            if (result < 0) {
    ...
    dbg(lvl_info, "RTL-SDR initialized: %.3f MHz, %d Hz, gain=%d, ppm=%d",
        hw->config.frequency_mhz, hw->config.sample_rate, 
        hw->config.gain, hw->config.ppm_correction);
...
int aprs_sdr_hw_detect_devices(int *count) { if (count) *count = 0; return 0; }
int aprs_sdr_hw_get_device_info(int index, char *name, size_t name_len,
                                enum aprs_sdr_device_type *type) { return 0; }
