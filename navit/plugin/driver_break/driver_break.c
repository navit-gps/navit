...
/* Append water points from list to stop->pois. */
static void driver_break_stop_add_water_pois(struct driver_break_stop *stop, GList *water_pois_list) {
...
/* Append cabin huts from list to stop->pois. */
static void driver_break_stop_add_cabin_pois(struct driver_break_stop *stop, GList *cabin_pois_list) {
    GList *poi_list = cabin_pois_list;
    while (poi_list) {
...
/* Fill stop with POIs from mapset or discover. */
static void driver_break_stop_fill_pois(struct driver_break_priv *priv, struct driver_break_stop *stop,
                                        const struct coord_geo *coord) {
    ...
    water_pois_list = poi_search_water_points_map(coord, priv->config.poi_water_search_radius_km, ms);
    cabin_pois_list =
        poi_search_cabins_map(coord, priv->config.poi_cabin_search_radius_km, ms, priv->config.enable_dnt_priority);
    driver_break_stop_add_water_pois(stop, water_pois_list);
    driver_break_stop_add_cabin_pois(stop, cabin_pois_list);
    poi_free_water_points(water_pois_list);
    poi_free_cabins(cabin_pois_list);
}

/* Convert one hiking stop to driver_break_stop and append to suggested_stops. */
static void process_one_hiking_stop(struct driver_break_priv *priv, struct hiking_driver_break_stop *h_stop) {
    struct coord c;
    struct driver_break_stop *stop;

    c = route_get_coord_dist(priv->current_route, (int)h_stop->position);
    ...
    driver_break_stop_fill_pois(priv, stop, &h_stop->coord);
...
static void process_hiking_stops(struct driver_break_priv *priv, GList *hiking_stops) {
    ...
    while (l) {
        ...
        if (h_stop)
            process_one_hiking_stop(priv, h_stop);
        l = g_list_next(l);
    }
}

/* Convert one cycling stop to driver_break_stop and append to suggested_stops. */
static void process_one_cycling_stop(struct driver_break_priv *priv, struct cycling_driver_break_stop *c_stop) {
    struct coord c;
    struct driver_break_stop *stop;

    c = route_get_coord_dist(priv->current_route, (int)c_stop->position);
    ...
    driver_break_stop_fill_pois(priv, stop, &c_stop->coord);
    priv->suggested_stops = g_list_append(priv->suggested_stops, stop);
}
...
static void process_cycling_stops(struct driver_break_priv *priv, GList *cycling_stops) {
    ...
    while (l) {
        ...
        if (c_stop)
            process_one_cycling_stop(priv, c_stop);
        l = g_list_next(l);
    }
}

static void driver_break_route_do_hiking(struct driver_break_priv *priv) {
...
static void driver_break_route_do_cycling(struct driver_break_priv *priv) {
    struct attr dist_attr;
    double total_distance = 0.0;
    GList *cycling_stops;
...
static void driver_break_route_do_car_truck(struct driver_break_priv *priv) {
...
static int driver_break_osd_set_attr(struct osd_priv *osd, struct attr *attr) { return 0; }
