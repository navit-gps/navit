...
static int driver_break_db_load_config_value(const char *key, int value, struct driver_break_config *config,
                                             int *loaded_count) {
    static struct config_key_map maps[] = {{"vehicle_type", NULL, 0, MAX_VEHICLE_TYPE, 1},
...
/* Parse and apply one config row. Updates loaded_count if value was applied. */
static void driver_break_db_apply_config_row(const char *key, const char *value_str, struct driver_break_config *config,
                                             int *loaded_count) {
    char *endptr;
    int value;
