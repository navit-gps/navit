...
/* Check if street type is suitable for rest stop */
static int is_suitable_highway_type(enum item_type type) {
    return (type == type_street_0 || type == type_street_service || type == type_track || type == type_street_3_land ||
            type == type_street_3_city);
}

/* Get highway type name from street item type */
static char *get_highway_type_name(enum item_type type) {
    if (type == type_street_0)
        return g_strdup("unclassified");
    if (type == type_street_service)
...
/* Create and add rest stop at coord_geo if valid. Returns 1 if added. */
static int try_add_rest_stop(GList **stops, int *count, struct coord_geo *coord_geo, struct driver_break_config *config,
                             struct item *street_item, double accumulated_distance_km) {
    ...
    stop->name = g_strdup_printf("Rest stop at %.1f km", accumulated_distance_km);
    stop->highway_type = get_highway_type_name(street_item->type);
...
GList *driver_break_finder_find_along_route(struct route *route, struct driver_break_config *config,
    ...
    while ((item = map_rect_get_item(mr)) && count < max_stops) {
        ...
        if (!is_suitable_highway_type(street_item->type) || accumulated_distance < 5000.0) {
            continue;
        }
        ...
        if (try_add_rest_stop(&stops, &count, &coord_geo, config, street_item, accumulated_distance / 1000.0)) {
