...
/* Create POI from node object JSON, or NULL if invalid. Caller frees POI. */
static struct driver_break_poi *parse_overpass_node(const char *obj_start, long node_id, double lat, double lon,
...
/* Find next node object in elements array. Returns obj_start or NULL. Updates *current. */
static const char *find_next_node_object(const char **current) {
...
static GList *parse_overpass_response(const char *json_response, struct coord_geo *center) {
    ...
    while (*current && *current != ']') {
        const char *obj_start = find_next_node_object(&current);
        if (!obj_start) {
            break;
        }
        ...
        struct driver_break_poi *poi = parse_overpass_node(obj_start, node_id, lat, lon, center);
        if (poi) {
...
GList *driver_break_poi_discover(struct coord_geo *center, int radius_km, const char **poi_categories,
    ...
    if (!poi_categories || num_categories == 0) {
        ...
        const char *general_categories[] = {"amenity=cafe", "amenity=restaurant", "tourism=museum",
                                            "tourism=viewpoint"};
