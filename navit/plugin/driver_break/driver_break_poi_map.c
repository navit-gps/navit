...
/* Check if item type is in the requested POI types list */
static int item_type_matches(enum item_type item_type, enum item_type *poi_types, int num_types) {
    int i;
    for (i = 0; i < num_types; i++) {
        if (item_type == poi_types[i]) {
            return 1;
        }
    }
    return 0;
...
/* Create POI from map item if within radius. Returns POI or NULL. */
static struct driver_break_poi *create_poi_from_item(struct item *item, struct coord_geo *center, double radius_m) {
...
GList *driver_break_poi_map_search(struct coord_geo *center, double radius_km, enum item_type *poi_types, int num_types,
    ...
    while (mapset_get_attr(ms, attr_map, &map_attr, iter)) {
        ...
        while ((item = map_rect_get_item(mr))) {
            if (!item_type_matches(item->type, poi_types, num_types)) {
                continue;
            }

            struct driver_break_poi *poi = create_poi_from_item(item, center, radius_m);
            if (poi) {
