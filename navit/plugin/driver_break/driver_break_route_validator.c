...
struct route_validation_result *route_validator_validate_motorcycle(struct route *route) {
    struct route_validation_result *result = g_new0(struct route_validation_result, 1);
    ...
    while ((item = map_rect_get_item(mr))) {
        ...
        if (route_validator_motorcycle_is_forbidden(street_item))
            forbidden_count++;
        if (route_validator_motorcycle_is_preferred(street_item))
            preferred_count++;
        if (!route_validator_motorcycle_road_surface_ok(street_item))
            bad_surface++;
        if (!route_validator_motorcycle_adventure_access_ok(street_item))
            access_denied++;
    }

    map_rect_destroy(mr);

    result->forbidden_segments = forbidden_count;
    if (forbidden_count > 0) {
        result->warnings = g_list_append(
            result->warnings, g_strdup_printf("Motorcycle: %d segment(s) with motorcycle=no", forbidden_count));
        result->is_valid = 0;
    }
    if (preferred_count > 0) {
        result->warnings = g_list_append(
            result->warnings,
            g_strdup_printf("Motorcycle: %d preferred segment(s) (motorcycle=yes/designated)", preferred_count));
    }
    if (bad_surface > 0) {
        result->warnings = g_list_append(
            result->warnings, g_strdup_printf("Motorcycle: %d segment(s) with non-paved surface", bad_surface));
    }
    if (access_denied > 0) {
        result->warnings = g_list_append(
            result->warnings, g_strdup_printf("Motorcycle: %d segment(s) with restricted access", access_denied));
        result->is_valid = 0;
    }
    if (segments > 0 && preferred_count == 0 && forbidden_count == 0) {
        result->warnings =
            g_list_append(result->warnings, g_strdup("Motorcycle: no explicitly preferred motorcycle ways on route"));
    }
