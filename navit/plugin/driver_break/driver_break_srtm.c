...
#define SRTM_VOID_VALUE (-32768)

/* Clamp lon/lat to tile bounds and compute pixel indices (0-3600). */
static void read_hgt_pixel_position(int lon_idx, int lat_idx, double lon, double lat, int *x_out, int *y_out) {
...
/* Read 16-bit big-endian elevation at byte offset; returns SRTM_VOID_VALUE on error. */
static int read_hgt_value_at_offset(FILE *f, long offset) {
    unsigned char bytes[2];
    int16_t elevation;

    if (fseek(f, offset, SEEK_SET) != 0)
        return SRTM_VOID_VALUE;
    if (fread(bytes, 1, 2, f) != 2)
        return SRTM_VOID_VALUE;
    elevation = (int16_t)((bytes[0] << 8) | bytes[1]);
    return (elevation == -32768) ? SRTM_VOID_VALUE : (int)elevation;
}
...
static int read_hgt_elevation(const char *filepath, int lon_idx, int lat_idx, double lon, double lat) {
    ...
    if (!f)
        return SRTM_VOID_VALUE;
    read_hgt_pixel_position(lon_idx, lat_idx, lon, lat, &x, &y);
    offset = (long)y * 3601L * 2L + (long)x * 2L;
    elevation = read_hgt_value_at_offset(f, offset);
    fclose(f);
    return elevation;
}
...
static struct srtm_region srtm_regions[] = {{"Europe", -12.97, 33.59, 34.15, 72.10, 0, 0, 0, 0},
...
/* Perform curl download to fp; try primary URL, then fallback if provided. Returns CURL result. */
static CURLcode srtm_curl_download(CURL *curl, FILE *fp, const char *url_primary, const char *url_fallback) {
    CURLcode res;

    curl_easy_setopt(curl, CURLOPT_URL, url_primary);
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, srtm_write_callback);
    curl_easy_setopt(curl, CURLOPT_WRITEDATA, fp);
    curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L);
    curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0L);
    res = curl_easy_perform(curl);
    if (res != CURLE_OK && url_fallback) {
        fseek(fp, 0, SEEK_SET);
...
/* Extract HGT from zip and verify size. Returns 1 on success. */
static int srtm_extract_and_verify(const char *filepath, const char *zip_path, const char *filename,
                                   struct srtm_tile *tile) {
    char *extract_cmd;
    int extract_result;
    GStatBuf stat_buf;

    extract_cmd = g_strdup_printf("cd '%s' && unzip -o -q '%s' '%s' 2>/dev/null", srtm_data_dir, zip_path, filename);
    ...
    if (stat_buf.st_size <= 20000000 || stat_buf.st_size >= 30000000) {
        dbg(lvl_warning, "SRTM: Extracted file has unexpected size: %ld bytes", (long)stat_buf.st_size);
        g_unlink(filepath);
        return 0;
    }
    tile->downloaded = 1;
...
static int srtm_download_tile(struct srtm_tile *tile, struct srtm_download_context *ctx) {
    ...
    res = srtm_curl_download(curl, fp, tile->url_primary, tile->url_fallback);
    fclose(fp);
    curl_easy_cleanup(curl);
    if (res == CURLE_OK && g_file_test(zip_path, G_FILE_TEST_EXISTS)) {
        ok = srtm_extract_and_verify(filepath, zip_path, tile->filename, tile);
        if (ok) {
            g_unlink(zip_path);
            g_free(zip_path);
            g_free(filepath);
