...
static int srtm_start_download_bbox(struct navit *nav, const char *label, double min_lon, double min_lat,
    ...
    srtm_ensure_init();
    region = srtm_region_from_bbox(label, min_lon, min_lat, max_lon, max_lat);
    if (!region) {
        navit_add_message(nav, "SRTM: could not build download region");
        return 0;
    }
...
static int driver_break_cmd_srtm_download_here(struct navit *nav, char *function, struct attr **in,
                                               struct attr ***out) {
    ...
    if (!navit_get_attr(nav, attr_position_coord_geo, &pos_attr, NULL) || !pos_attr.u.coord_geo) {
        navit_add_message(nav, "SRTM: no vehicle position for download");
        return 0;
    }

    cg = pos_attr.u.coord_geo;
    return srtm_start_download_bbox(nav, "here", floor(cg->lng) - 0.5, floor(cg->lat) - 0.5, floor(cg->lng) + 1.5,
                                    floor(cg->lat) + 1.5)
               ? 1
               : 0;
}

static int driver_break_cmd_srtm_download_route(struct navit *nav, char *function, struct attr **in,
                                                struct attr ***out) {
    ...
    if (!navit_get_attr(nav, attr_route, &route_attr, NULL) || !route_attr.u.route) {
        navit_add_message(nav, "SRTM: no active route for download");
        return 0;
    }

    route = route_attr.u.route;
    if (!driver_break_route_bbox(route, &min_lon, &min_lat, &max_lon, &max_lat)) {
        navit_add_message(nav, "SRTM: could not determine route bounding box");
        return 0;
    }
    ...
    max_lon += 0.1;
    max_lat += 0.1;
    return srtm_start_download_bbox(nav, "route", min_lon, min_lat, max_lon, max_lat) ? 1 : 0;
}

static int driver_break_cmd_srtm_download_view(struct navit *nav, char *function, struct attr **in,
                                               struct attr ***out) {
    struct attr pos_attr;
    struct coord_geo *cg;
    ...
    if (!navit_get_attr(nav, attr_position_coord_geo, &pos_attr, NULL) || !pos_attr.u.coord_geo) {
        navit_add_message(nav, "SRTM: no position for map-view download (using vehicle position)");
        return 0;
    }

    cg = pos_attr.u.coord_geo;
    return srtm_start_download_bbox(nav, "view", floor(cg->lng) - 1.0, floor(cg->lat) - 1.0, floor(cg->lng) + 2.0,
                                    floor(cg->lat) + 2.0)
               ? 1
               : 0;
}
...
static struct command_table driver_break_srtm_commands[] = {
    ...
    {"srtm_download_here",   command_cast(driver_break_cmd_srtm_download_here)  },
    {"srtm_download_route",  command_cast(driver_break_cmd_srtm_download_route) },
    {"srtm_download_view",   command_cast(driver_break_cmd_srtm_download_view)  },
