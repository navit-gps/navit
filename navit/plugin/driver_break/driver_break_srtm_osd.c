...
static int driver_break_cmd_srtm_download_menu(struct navit *nav, char *function, struct attr **in,
    ...
    if (regions) {
        ...
        while (l) {
            ...
            if (region) {
                dbg(lvl_info, "  - %s: %d tiles, %.1f MB, %d%% complete", region->name, region->tile_count,
                    region->size_bytes / (1024.0 * 1024.0), region->progress_percent);
            }
            l = g_list_next(l);
        }
...
static int driver_break_cmd_srtm_download_region(struct navit *nav, char *function, struct attr **in,
    ...
    dbg(lvl_info, "Driver Break plugin: SRTM download started for %s (%d tiles, %.1f MB)", region->name,
        region->tile_count, region->size_bytes / (1024.0 * 1024.0));
