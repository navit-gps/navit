 ...
 * Returns 0 on success and 1 on failure so the caller can propagate to main()'s
 * failure counter. TEST_ASSERT performs `return 1`, so this must return int. */
static int srtm_test_assert_rondane(int elev1, int elev2, int elev3) {
    if (elev1 != SRTM_VOID)
...
static int test_srtm_elevation_hiking_route(void) {
    ...
    int rc = srtm_test_assert_rondane(elev1, elev2, elev3);
    g_free(test_dir);
    return rc;
}
