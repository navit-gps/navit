...
static struct mapset *load_test_map_data(void) {
    ...
    if (!g_file_test(map_file, G_FILE_TEST_EXISTS)) {
        ...
        map_file =
            g_build_filename("/mnt/2e9a1e9f-2097-408c-ab9a-a01b32f11d28/github-projects/navit", "navit", "plugin",
                             "driver_break", "tests", "map_data", "osm_bbox_7.2,60.8,11.0,61.3.bin", NULL);
    }
    if (!g_file_test(map_file, G_FILE_TEST_EXISTS)) {
        g_free(map_file);
