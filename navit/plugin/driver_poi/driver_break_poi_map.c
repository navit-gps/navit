...
static struct map_selection *poi_map_new_center_selection(struct coord_geo *center, double radius_km) {
    struct coord c;
    ...
    transform_from_geo(projection_mg, center, &c);
    radius_internal = (int)(radius_m * 9.0);
    rect.lu = c;
    rect.rl = c;
    rect.lu.x -= radius_internal;
    rect.lu.y += radius_internal;
...
static GList *poi_map_collect_fuel_from_map(struct map *map, struct map_selection *sel, struct coord_geo *center,
                                            double radius_m, enum driver_break_vehicle_type vehicle_type,
                                            int fuel_type) {
    GList *result = NULL;
    struct map_rect *mr = map_rect_new(map, sel);
    struct item *item;
    ...
    while ((item = map_rect_get_item(mr))) {
        ...
        if (!item_coord_get(item, &item_coord, 1))
            continue;
        transform_to_geo(projection_mg, &item_coord, &item_geo);
        distance = driver_break_coord_distance_geo(center, &item_geo);
        if (distance > radius_m)
            continue;
        if (!fuel_station_matches_profile(item, vehicle_type, fuel_type))
            continue;
        poi = poi_from_map_item(item, &item_geo, distance);
...
GList *driver_break_poi_map_search_fuel(struct coord_geo *center, double radius_km, struct mapset *ms,
    ...
    sel = poi_map_new_center_selection(center, radius_km);
    iter = mapset_attr_iter_new(NULL);

    while (mapset_get_attr(ms, attr_map, &map_attr, iter)) {
        GList *found =
            poi_map_collect_fuel_from_map(map_attr.u.map, sel, center, radius_km * 1000.0, vehicle_type, fuel_type);
        if (found)
            result = g_list_concat(result, found);
    }

    mapset_attr_iter_destroy(iter);
    g_free(sel);
    return g_list_sort(result, driver_break_poi_compare_distance);
}
