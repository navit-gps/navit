...
static const char *j1850_init_strings[] = {"ATZ\r\n",  "ATI\r\n",  "ATL1\r\n", "ATH1\r\n",
                                           "ATS1\r\n", "ATAL\r\n", "ATMA\r\n", NULL};

/* Returns 1 if still in init phase (caller should return), 0 otherwise. */
static int j1850_send_init_commands(struct j1850 *j1850) {
    if (!j1850_init_strings[j1850->init_string_index])
        return 0;
    dbg(lvl_info, "Sending next init command : %s", j1850_init_strings[j1850->init_string_index]);
    if (j1850->device > 0)
        write_to_serial_port((unsigned char *)j1850_init_strings[j1850->init_string_index++], j1850->device);
    if (!j1850_init_strings[j1850->init_string_index]) {
...
static void j1850_handle_header_10(struct j1850 *j1850) {
    char *w1 = g_strndup(j1850->message + 2, 2);
    char *w2 = g_strndup(j1850->message + 4, 2);
    char *w3 = g_strndup(j1850->message + 6, 2);
    if (w1 && w2 && w3) {
...
static void j1850_handle_header_3d(struct j1850 *j1850, struct attr *navit) {
    if (strcmp(j1850->message, "3D110000EE") == 0)
        return;
    if (strcmp(j1850->message, "3D1120009B") == 0) {
        dbg(lvl_error, "L1");
        command_evaluate(navit, "gui.spotify_volume_up()");
        return;
    }
    if (strcmp(j1850->message, "3D110080C8") == 0) {
        dbg(lvl_error, "L2");
        command_evaluate(navit, "gui.spotify_volume_toggle()");
        return;
    }
    if (strcmp(j1850->message, "3D1110005A") == 0) {
        dbg(lvl_error, "L3");
        command_evaluate(navit, "gui.spotify_volume_down()");
        return;
    }
    if (strcmp(j1850->message, "3D110400C3") == 0) {
        dbg(lvl_error, "R1");
        command_evaluate(navit, "gui.spotify_next_track()");
        return;
    }
    if (strcmp(j1850->message, "3D110002D4") == 0) {
        dbg(lvl_error, "R2");
        command_evaluate(navit, "gui.spotify_toggle()");
        return;
    }
    if (strcmp(j1850->message, "3D11020076") == 0) {
        dbg(lvl_error, "R3");
...
static void j1850_handle_message(struct j1850 *j1850) {
    char header[3];
    struct attr navit;
    ...
    if (strncmp(header, "10", 2) == 0)
        j1850_handle_header_10(j1850);
    else if (strncmp(header, "3D", 2) == 0)
        j1850_handle_header_3d(j1850, &navit);
    else if (strncmp(header, "72", 2) == 0) {
        char *data = g_strndup(j1850->message + 2, 8);
        if (data) {
            j1850->odo = ((int)strtol(data, NULL, 16)) / 8000;
            g_free(data);
        }
    } else if (strncmp(header, "A4", 2) == 0) {
        char *w1 = g_strndup(j1850->message + 2, 4);
        if (w1) {
...
static void j1850_handle_char(struct j1850 *j1850, char buf) {
    int value = buf - 48;
...
static void j1850_idle(struct j1850 *j1850) {
    ...
    if (j1850_send_init_commands(j1850))
        return;
    if (j1850->device < 0) {
    ...
    if (buf == 13) {
        ...
        j1850_handle_message(j1850);
        return;
    }
    j1850_handle_char(j1850, buf);
}
...
static void osd_j1850_init(struct j1850 *this, struct navit *nav) {
    ...
    // navit_add_callback(nav, this->click_cb = callback_new_attr_1(callback_cast (osd_j1850_click), attr_button,
    // this));
