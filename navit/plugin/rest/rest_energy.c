...
void energy_model_init(struct energy_model *model, double totalweight, 
    ...
    model->cost0 = (model->pw + model->p_standby) / model->vmax + 
                   model->f_roll + model->f_air * model->vmax * model->vmax;
}
...
double energy_get_effective_speed(struct energy_model *model, double speed_limit) {
    if (!model) return speed_limit;
    if (speed_limit > model->vmax) return model->vmax;
    return speed_limit;
}
