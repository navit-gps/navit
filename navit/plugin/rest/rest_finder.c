...
int rest_finder_is_valid_location(struct coord_geo *coord, struct rest_config *config) {
...
int rest_finder_is_valid_nightly_location(struct coord_geo *coord, 
    ...
    if (!rest_finder_is_valid_location(coord, config)) {
        return 0;
    }
...
GList *rest_finder_find_along_route(struct route *route, 
    ...
    if (!route_map) {
        dbg(lvl_warning, "Rest plugin: No route map available");
        return NULL;
    }
    
    dbg(lvl_info, "Rest plugin: Finding rest stops along route");
            ...
            if (street_item) {
                ...
                if (item_coord_get(item, &c, 1)) {
                    ...
                    if (is_suitable && accumulated_distance >= 5000.0) {
                        ...
                        /* Validate location */
                        if (rest_finder_is_valid_location(&coord_geo, config)) {
                            stop = g_new0(struct rest_stop, 1);
                            ...
                            stop->pois = rest_poi_discover(&coord_geo, config->poi_search_radius_km,
                                                           NULL, 0);
    ...
    dbg(lvl_info, "Rest plugin: Found %d rest stop candidates", count);
...
GList *rest_finder_find_near(struct coord_geo *center, 
    ...
    dbg(lvl_info, "Rest plugin: Finding rest stops near %.6f,%.6f (radius %.1f km)",
    ...
    GList *candidates = rest_poi_discover(center, (int)distance_km, 
                                         poi_categories, 
    ...
    if (candidates) {
        ...
        while (l && count < max_stops) {
            struct rest_poi *candidate = (struct rest_poi *)l->data;
            if (candidate) {
                /* Validate location */
                if (rest_finder_is_valid_location(&candidate->coord, config)) {
                    struct rest_stop *stop = g_new0(struct rest_stop, 1);
                    stop->coord = candidate->coord;
                    stop->distance_from_route = candidate->distance_from_rest_stop;
                    ...
                    stop->pois = rest_poi_discover(&candidate->coord, 
...
void rest_finder_free_list(GList *stops) {
    GList *l = stops;
    while (l) {
        rest_finder_free((struct rest_stop *)l->data);
        l = g_list_next(l);
    }
    g_list_free(stops);
}
