...
static void resolve_gui_internal_functions(void) {
    ...
    for (i = 0; lib_paths[i] != NULL; i++) {
        ...
        if (handle) {
            dbg(lvl_debug, "Rest plugin: Opened gui_internal library: %s", lib_paths[i]);
            break;
        }
        error = dlerror();
        if (error) {
            dbg(lvl_debug, "Rest plugin: Failed to open %s: %s", lib_paths[i], error);
        }
    }
    
    /* If we couldn't open explicitly, try RTLD_DEFAULT */
    if (!handle) {
        dbg(lvl_debug, "Rest plugin: Could not open gui_internal library explicitly, trying RTLD_DEFAULT");
        handle = RTLD_DEFAULT;
    }
    ...
    error = dlerror();
    if (error) {
        dbg(lvl_debug, "Rest plugin: dlerror after symbol resolution: %s", error);
    }
    
    if (!gui_internal_menu_render_ptr) {
        dbg(lvl_error, "Rest plugin: Failed to resolve gui_internal functions - internal GUI may not be available");
        dbg(lvl_debug, "Rest plugin: Resolved: back=%p enter=%p menu=%p render=%p", 
            gui_internal_back_ptr, gui_internal_enter_ptr, gui_internal_menu_ptr, gui_internal_menu_render_ptr);
    } else {
        dbg(lvl_debug, "Rest plugin: Successfully resolved all gui_internal functions");
    }
}
...
int rest_cmd_suggest_stop(struct navit *nav, char *function, struct attr **in, struct attr ***out);
int rest_cmd_show_history(struct navit *nav, char *function, struct attr **in, struct attr ***out);
int rest_cmd_configure(struct navit *nav, char *function, struct attr **in, struct attr ***out);
int rest_cmd_start_break(struct navit *nav, char *function, struct attr **in, struct attr ***out);
int rest_cmd_end_break(struct navit *nav, char *function, struct attr **in, struct attr ***out);
int rest_cmd_configure_intervals(struct navit *nav, char *function, struct attr **in, struct attr ***out);
int rest_cmd_configure_overnight(struct navit *nav, char *function, struct attr **in, struct attr ***out);

static struct command_table rest_commands[] = {
    {"rest_suggest_stop", command_cast(rest_cmd_suggest_stop)},
    {"rest_show_history", command_cast(rest_cmd_show_history)},
    {"rest_configure", command_cast(rest_cmd_configure)},
    {"rest_start_break", command_cast(rest_cmd_start_break)},
    {"rest_end_break", command_cast(rest_cmd_end_break)},
    {"rest_configure_intervals", command_cast(rest_cmd_configure_intervals)},
    {"rest_configure_overnight", command_cast(rest_cmd_configure_overnight)},
};

/* Register commands in rest_osd_new */
void rest_register_commands(struct navit *nav) {
    struct attr gui_attr;
    struct attr cbl_attr;
    struct attr cmd_attr;
    
    if (!nav) {
        dbg(lvl_error, "Rest plugin: Cannot register commands - nav is NULL");
        return;
    }
    
    /* Get GUI from navit - commands are called from GUI context */
    if (!navit_get_attr(nav, attr_gui, &gui_attr, NULL)) {
        dbg(lvl_error, "Rest plugin: navit does not have a GUI");
        return;
    }
    
    /* Get GUI's callback list */
    if (!gui_get_attr(gui_attr.u.gui, attr_callback_list, &cbl_attr, NULL)) {
        dbg(lvl_error, "Rest plugin: GUI does not have attr_callback_list");
        return;
    }
    dbg(lvl_debug, "Rest plugin: Found GUI callback_list at %p", cbl_attr.u.callback_list);
    
    /* Create command table callback */
    command_add_table_attr(rest_commands, sizeof(rest_commands)/sizeof(struct command_table), 
                          nav, &cmd_attr);
    
    if (cmd_attr.type != attr_callback) {
        dbg(lvl_error, "Rest plugin: command_add_table_attr failed - attr type is %d, expected %d", 
            cmd_attr.type, attr_callback);
        return;
    }
    
    /* Add callback to GUI's callback list (not navit's) */
    dbg(lvl_debug, "Rest plugin: Adding callback %p to GUI callback list", cmd_attr.u.callback);
    callback_list_add(cbl_attr.u.callback_list, cmd_attr.u.callback);
    
    dbg(lvl_info, "Rest plugin: Menu commands registered (count=%zu)", 
        sizeof(rest_commands)/sizeof(struct command_table));
    dbg(lvl_debug, "Rest plugin: Registered commands: rest_configure_intervals, rest_configure_overnight");
}

/* Static pointer to the rest_priv instance (there should only be one) */
/* This is set in rest_osd_new and accessed by command handlers */
struct rest_priv *rest_plugin_instance = NULL;

/* Get rest plugin instance - returns the static instance */
static struct rest_priv *rest_get_plugin(struct navit *nav) {
    (void)nav; /* Unused, but kept for API consistency */
    if (!rest_plugin_instance) {
        dbg(lvl_warning, "Rest plugin: Plugin instance not found - OSD may not be instantiated");
    }
    return rest_plugin_instance;
}
...
static struct gui_priv *rest_get_internal_gui_priv(struct navit *nav) {
    struct gui *gui;
...
int rest_cmd_suggest_stop(struct navit *nav, char *function, struct attr **in, struct attr ***out) {
    dbg(lvl_info, "Rest plugin: Suggest rest stop command");
...
int rest_cmd_show_history(struct navit *nav, char *function, struct attr **in, struct attr ***out) {
    dbg(lvl_info, "Rest plugin: Show history command");
...
/* Configure command */
int rest_cmd_configure(struct navit *nav, char *function, struct attr **in, struct attr ***out) {
    dbg(lvl_info, "Rest plugin: Configure command");
...
/* Start break command */
int rest_cmd_start_break(struct navit *nav, char *function, struct attr **in, struct attr ***out) {
    void *plugin = rest_get_plugin(nav);
    if (plugin) {
        /* Record break start time */
        dbg(lvl_info, "Rest plugin: Break started");
    }
    return 1;
}
...
int rest_cmd_end_break(struct navit *nav, char *function, struct attr **in, struct attr ***out) {
    ...
    if (plugin) {
        ...
        dbg(lvl_info, "Rest plugin: Break ended");
    }
    return 1;
}

/* Callback to save configuration after dialog interaction */
static void rest_save_config_callback(struct gui_priv *gui_priv, struct widget *widget, void *data) {
    struct rest_priv *priv = (struct rest_priv *)data;
    if (priv && priv->db) {
        rest_db_save_config(priv->db, &priv->config);
        navit_add_message(priv->nav, "Rest plugin: Configuration saved");
    }
    gui_internal_back(gui_priv, NULL, NULL);
}

/* Show car interval configuration dialog */
static void rest_show_car_intervals_dialog(struct gui_priv *gui_priv, struct rest_priv *priv) {
    struct widget *menu, *box, *label, *button;
    char buffer[256];
    
    if (!priv) {
        dbg(lvl_error, "Rest plugin: rest_show_car_intervals_dialog called with NULL priv");
        return;
    }
    
    dbg(lvl_debug, "Rest plugin: Showing car intervals dialog - config values: soft_limit=%d, max=%d, break_interval=%d, break_duration=%d",
    ...
    /* Soft limit hours - validate before displaying */
    {
        ...
        if (val < 0 || val > 12) {
            dbg(lvl_error, "Rest plugin: Invalid car_soft_limit_hours value: %d, using default 7", val);
            val = 7;
    ...
    /* Max hours - validate before displaying */
    {
        ...
        if (val < 0 || val > 12) {
            dbg(lvl_error, "Rest plugin: Invalid car_max_hours value: %d, using default 10", val);
            val = 10;
    ...
    /* Break interval - validate before displaying */
    {
        ...
        if (val < 0 || val > 12) {
            dbg(lvl_error, "Rest plugin: Invalid car_break_interval_hours value: %d, using default 4", val);
            val = 4;
    ...
    /* Break duration - validate before displaying */
    {
        ...
        if (val < 0 || val > 120) {
            dbg(lvl_error, "Rest plugin: Invalid car_break_duration_min value: %d, using default 30", val);
            val = 30;
    ...
    button = gui_internal_button_new_with_callback(gui_priv, "OK", NULL, 
        gravity_center|flags_fill, rest_save_config_callback, priv);
...
static void rest_show_truck_intervals_dialog(struct gui_priv *gui_priv, struct rest_priv *priv) {
    struct widget *menu, *box, *label, *button;
    char buffer[256];
...
static void rest_show_hiking_intervals_dialog(struct gui_priv *gui_priv, struct rest_priv *priv) {
    struct widget *menu, *box, *label, *button;
    char buffer[256];
    ...
    /* Main rest distance */
    snprintf(buffer, sizeof(buffer), "Main rest distance: %.2f km", priv->config.hiking_rest_distance_main);
    label = gui_internal_label_new(gui_priv, buffer);
    gui_internal_widget_append(box, label);
    
    /* Alternative rest distance */
    snprintf(buffer, sizeof(buffer), "Alternative rest: %.2f km", priv->config.hiking_rest_distance_alt);
    label = gui_internal_label_new(gui_priv, buffer);
    gui_internal_widget_append(box, label);
...
static void rest_show_cycling_intervals_dialog(struct gui_priv *gui_priv, struct rest_priv *priv) {
    struct widget *menu, *box, *label, *button;
    char buffer[256];
    ...
    /* Main rest distance */
    snprintf(buffer, sizeof(buffer), "Main rest distance: %.2f km", priv->config.cycling_rest_distance_main);
    label = gui_internal_label_new(gui_priv, buffer);
    gui_internal_widget_append(box, label);
    
    /* Alternative rest distance */
    snprintf(buffer, sizeof(buffer), "Alternative rest: %.2f km", priv->config.cycling_rest_distance_alt);
    label = gui_internal_label_new(gui_priv, buffer);
    gui_internal_widget_append(box, label);
...
int rest_cmd_configure_intervals(struct navit *nav, char *function, struct attr **in, struct attr ***out) {
    ...
    dbg(lvl_info, "Rest plugin: rest_cmd_configure_intervals called");
    plugin = rest_get_plugin(nav);
    if (!plugin) {
        dbg(lvl_error, "Rest plugin: Plugin not found - OSD may not be instantiated. Check if <osd type=\"rest\" enabled=\"yes\"/> is in config.");
        navit_add_message(nav, "Rest plugin: Plugin not loaded. Please check configuration.");
        return 0;
    }

    priv = (struct rest_priv *)plugin;

    /* Get current vehicle profile */
    profile = navit_get_vehicleprofile(nav);
    if (!profile) {
        dbg(lvl_warning, "Rest plugin: No vehicle profile found");
        navit_add_message(nav, "Rest plugin: No vehicle profile selected");
        return 0;
    }

    if (!vehicleprofile_get_attr(profile, attr_name, &profile_attr, NULL)) {
        dbg(lvl_warning, "Rest plugin: Could not get profile name");
        navit_add_message(nav, "Rest plugin: Could not get profile name");
        return 0;
    }

    profile_name = profile_attr.u.str;
    dbg(lvl_info, "Rest plugin: Configure intervals for profile: %s", profile_name);

    /* Get internal GUI priv - if not available, show message */
    gui_priv = rest_get_internal_gui_priv(nav);
    if (!gui_priv) {
        navit_add_message(nav, "Rest plugin: Configuration dialog requires internal GUI");
        dbg(lvl_warning, "Rest plugin: Internal GUI not available");
        return 0;
    }

    /* Show configuration dialog based on profile */
    if (!g_ascii_strcasecmp(profile_name, "car")) {
        rest_show_car_intervals_dialog(gui_priv, priv);
    } else if (!g_ascii_strcasecmp(profile_name, "truck") || !g_ascii_strcasecmp(profile_name, "Truck")) {
        rest_show_truck_intervals_dialog(gui_priv, priv);
    } else if (!g_ascii_strcasecmp(profile_name, "hiking") || !g_ascii_strcasecmp(profile_name, "pedestrian")) {
        rest_show_hiking_intervals_dialog(gui_priv, priv);
    } else if (!g_ascii_strcasecmp(profile_name, "bike") || !g_ascii_strcasecmp(profile_name, "cycling")) {
        rest_show_cycling_intervals_dialog(gui_priv, priv);
    } else {
        dbg(lvl_warning, "Rest plugin: Unknown profile type: %s", profile_name);
        navit_add_message(nav, "Rest plugin: Configuration not available for this profile");
        return 0;
    }
...
static void rest_show_overnight_dialog(struct gui_priv *gui_priv, struct rest_priv *priv, const char *profile_name) {
    struct widget *menu, *box, *label, *button;
    char buffer[256];
    char title[128];
    
    if (!priv) {
        dbg(lvl_error, "Rest plugin: rest_show_overnight_dialog called with NULL priv");
        return;
    }
    
    dbg(lvl_debug, "Rest plugin: Showing overnight dialog - config values: min_buildings=%d, min_glaciers=%d, poi_radius=%d, water_radius=%d, cabin_radius=%d",
    ...
    /* Min distance from buildings - validate before displaying */
    {
        ...
        if (val < 0 || val > 10000) {
            dbg(lvl_error, "Rest plugin: Invalid min_distance_from_buildings value: %d, using default 150", val);
            val = 150;
    ...
    if (!g_ascii_strcasecmp(profile_name, "hiking") || !g_ascii_strcasecmp(profile_name, "pedestrian")) {
        ...
        if (val < 0 || val > 10000) {
            dbg(lvl_error, "Rest plugin: Invalid min_distance_from_glaciers value: %d, using default 300", val);
            val = 300;
    ...
    /* POI search radius - validate before displaying */
    {
        ...
        if (val < 0 || val > 1000) {
            dbg(lvl_error, "Rest plugin: Invalid poi_search_radius_km value: %d, using default 15", val);
            val = 15;
    ...
    /* Water search radius - validate before displaying */
    {
        ...
        if (val < 0 || val > 100) {
            dbg(lvl_error, "Rest plugin: Invalid poi_water_search_radius_km value: %d, using default 2", val);
            val = 2;
    ...
    /* Cabin search radius - validate before displaying */
    {
        ...
        if (val < 0 || val > 100) {
            dbg(lvl_error, "Rest plugin: Invalid poi_cabin_search_radius_km value: %d, using default 5", val);
            val = 5;
...
int rest_cmd_configure_overnight(struct navit *nav, char *function, struct attr **in, struct attr ***out) {
    ...
    dbg(lvl_info, "Rest plugin: rest_cmd_configure_overnight called");
    plugin = rest_get_plugin(nav);
    if (!plugin) {
        dbg(lvl_error, "Rest plugin: Plugin not found - OSD may not be instantiated. Check if <osd type=\"rest\" enabled=\"yes\"/> is in config.");
        navit_add_message(nav, "Rest plugin: Plugin not loaded. Please check configuration.");
    ...
    profile_name = profile_attr.u.str;
    dbg(lvl_info, "Rest plugin: Configure overnight stops for profile: %s", profile_name);
    ...
    rest_show_overnight_dialog(gui_priv, priv, profile_name);
