...
static int poi_compare_water_distance(const struct water_point *a, 
    ...
    if (a->distance_from_rest_stop < b->distance_from_rest_stop) return -1;
    if (a->distance_from_rest_stop > b->distance_from_rest_stop) return 1;
    return 0;
}
...
GList *poi_search_water_points(struct coord_geo *rest_stop, double radius_km) {
    ...
    GList *pois = rest_poi_discover(rest_stop, (int)radius_km, 
                                    water_categories, 
    ...
    if (pois) {
        ...
        while (l) {
            ...
            if (poi) {
                ...
                wp->distance_from_rest_stop = coord_distance_poi(rest_stop, &poi->coord);
...
GList *poi_search_water_points_map(struct coord_geo *rest_stop, double radius_km, struct mapset *ms) {
    ...
    if (!rest_stop || !ms || radius_km <= 0) {
        return NULL;
    }
    
    /* Use map-based POI search */
    GList *pois = rest_poi_map_search_water(rest_stop, radius_km, ms);
    if (pois) {
        GList *l = pois;
        while (l) {
            struct rest_poi *poi = (struct rest_poi *)l->data;
    ...
    if (pois) {
        ...
        while (l) {
            ...
            if (poi) {
                ...
                cabin->distance_from_rest_stop = coord_distance_poi(rest_stop, &poi->coord);
...
GList *poi_search_cabins_map(struct coord_geo *rest_stop, double radius_km, struct mapset *ms, int enable_dnt_priority) {
    ...
    GList *pois = rest_poi_map_search_cabins(rest_stop, search_radius, ms);
    if (pois) {
        GList *l = pois;
        while (l) {
            struct rest_poi *poi = (struct rest_poi *)l->data;
