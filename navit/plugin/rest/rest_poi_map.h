...
#ifndef NAVIT_PLUGIN_REST_POI_MAP_H
#define NAVIT_PLUGIN_REST_POI_MAP_H
...
GList *rest_poi_map_search(struct coord_geo *center, double radius_km, 
...
GList *rest_poi_map_search_car_pois(struct coord_geo *center, double radius_km, struct mapset *ms);

/* Check if POI item has DNT/network tags */
int rest_poi_is_network_cabin(struct item *item, char **network_name);

/* Check if route segment has pilgrimage/hiking route tags */
int rest_route_is_pilgrimage_hiking(struct item *item);

#endif /* NAVIT_PLUGIN_REST_POI_MAP_H */
