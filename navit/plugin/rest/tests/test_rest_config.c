 ...
 *
 * Unit tests for Rest plugin configuration and driving time calculations
 */
...
static int test_rest_interval_range(void) {
    struct rest_config config;
    rest_config_default(&config);
    
    TEST_ASSERT(config.rest_interval_min_hours == 1, "Min rest interval should be 1 hour");
    TEST_ASSERT(config.rest_interval_max_hours == 6, "Max rest interval should be 6 hours");
    TEST_ASSERT(config.rest_interval_min_hours < config.rest_interval_max_hours, 
...
int main(void) {
    ...
    printf("Running Rest plugin configuration tests...\n");
    ...
    failures += test_rest_interval_range();
