 ...
 *
 * Unit tests for Rest plugin database operations
 */
...
static char *create_temp_db(void) {
    char template[] = "/tmp/rest_test_db_XXXXXX";
...
static int test_db_create_destroy(void) {
    ...
    struct rest_db *db = rest_db_new(db_path);
...
static int test_db_add_history(void) {
    ...
    struct rest_stop_history entry;
    ...
    int result = rest_db_add_history(db, &entry);
...
static int test_db_get_history(void) {
    ...
    rest_db_add_history(db, &entry);
    
    GList *history = rest_db_get_history(db, now - 100);
    TEST_ASSERT(history != NULL, "History retrieval failed");
    TEST_ASSERT(g_list_length(history) == 1, "Wrong number of history entries");
    
    struct rest_stop_history *retrieved = (struct rest_stop_history *)history->data;
    ...
    GList *l = history;
    while (l) {
        struct rest_stop_history *h = (struct rest_stop_history *)l->data;
        g_free(h->name);
        g_free(h);
        l = g_list_next(l);
    }
...
static int test_db_clear_history(void) {
    ...
    time_t now = time(NULL);
    struct rest_stop_history entry1, entry2;
    ...
    rest_db_add_history(db, &entry1);
    rest_db_add_history(db, &entry2);
    
    /* Clear entries older than now - 100 */
    int result = rest_db_clear_history(db, now - 100);
    TEST_ASSERT(result == 1, "Clear history failed");
    
    GList *history = rest_db_get_history(db, 0);
...
static int test_db_save_load_config(void) {
    ...
    int result = rest_db_save_config(db, &config);
    TEST_ASSERT(result == 1, "Config save failed");
    
    struct rest_config loaded_config;
    memset(&loaded_config, 0, sizeof(loaded_config));
    result = rest_db_load_config(db, &loaded_config);
...
int main(void) {
    ...
    printf("Running Rest plugin database tests...\n");
