 ...
 *
 * Unit tests for Rest plugin rest stop finder
 */
...
static int test_coord_validation(void) {
    ...
    int result = rest_finder_is_valid_location(&valid_coord, &config);
    ...
    result = rest_finder_is_valid_location(&invalid_lat, &config);
    ...
    result = rest_finder_is_valid_location(&invalid_lng, &config);
    TEST_ASSERT(result == 0, "Invalid longitude should fail validation");
    
    /* NULL coordinate */
    result = rest_finder_is_valid_location(NULL, &config);
...
static int test_rest_stop_free(void) {
    ...
    rest_finder_free(stop);
    /* If we get here without crash, free worked */
    
    return 0;
}

static int test_rest_stop_list_free(void) {
    GList *stops = NULL;
    
    struct rest_stop *stop1 = g_new0(struct rest_stop, 1);
    stop1->name = g_strdup("Stop 1");
    stop1->coord.lng = -122.4194;  /* Longitude first */
    stop1->coord.lat = 37.7749;    /* Latitude second */
    
    struct rest_stop *stop2 = g_new0(struct rest_stop, 1);
    ...
    rest_finder_free_list(stops);
    /* If we get here without crash, free list worked */
    
    return 0;
}

static int test_rest_finder_near_null_params(void) {
    struct rest_config config;
    memset(&config, 0, sizeof(config));
    
    /* NULL center should return NULL */
    GList *result = rest_finder_find_near(NULL, 10.0, &config);
    ...
    result = rest_finder_find_near(&center, 10.0, NULL);
...
int main(void) {
    ...
    printf("Running Rest plugin finder tests...\n");
    
    failures += test_coord_validation();
    failures += test_distance_calculation();
    failures += test_rest_stop_free();
    failures += test_rest_stop_list_free();
    failures += test_rest_finder_near_null_params();
