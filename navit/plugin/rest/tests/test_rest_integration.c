 ...
 *
 * Integration tests for Rest plugin using actual Navit and plugin functions
 */
...
static int test_hiking_rest_intervals(void) {
    ...
    GList *stops = hiking_calculate_rest_stops_with_max(total_distance, 0, 40000.0);
    ...
    GList *l = stops;
    struct hiking_rest_stop *first_stop = (struct hiking_rest_stop *)l->data;
        ...
        if (stop) {
            ...
            if (prev_position > 0) {
                ...
                TEST_ASSERT(interval > 11000.0 && interval < 12000.0, 
                           "Rest intervals should be ~11.3 km");
            }
    ...
    hiking_free_rest_stops(stops);
    return 0;
}

/* Test cycling rest stop calculation */
static int test_cycling_rest_intervals(void) {
    /* Test with 100 km route */
    double total_distance = 100000.0;  /* 100 km in meters */
    
    /* Calculate rest stops using actual plugin function */
    GList *stops = cycling_calculate_rest_stops(total_distance, 0);
...
static int test_poi_discovery_car_route(void) {
    ...
    /* Note: This will use Overpass API fallback if mapset not available */
    GList *pois = rest_poi_discover(&moelv, 5, NULL, 0);
    
    if (pois) {
        rest_poi_free_list(pois);
    }
...
static int test_route_validation_functions(void) {
    ...
    TEST_ASSERT(route_validator_is_forbidden_highway("motorway") == 1, 
                "Motorway should be forbidden for hiking");
    TEST_ASSERT(route_validator_is_forbidden_highway("trunk") == 1, 
                "Trunk should be forbidden for hiking");
    TEST_ASSERT(route_validator_is_forbidden_highway("primary") == 1, 
                "Primary should be forbidden for hiking");
    
    /* Test priority path detection */
    TEST_ASSERT(route_validator_is_priority_path("footway") == 1, 
                "Footway should be priority path");
    TEST_ASSERT(route_validator_is_priority_path("path") == 1, 
                "Path should be priority path");
    TEST_ASSERT(route_validator_is_priority_path("track") == 1, 
                "Track should be priority path");
    
    return 0;
}

/* Test rest stop creation along route */
static int test_rest_stop_creation(void) {
    ...
    /* For 50 km with rest_distance=11.295 km: 
    ...
    GList *hiking_stops = hiking_calculate_rest_stops_with_max(route_distance, 0, 40000.0);
...
static int test_poi_discovery_integration(void) {
    ...
    struct coord_geo rest_stop = {60.9, 7.3};  /* Near Bjordalsbu */
    
    /* Test water point discovery */
    GList *water = poi_search_water_points(&rest_stop, 2.0);
    /* Function should execute (may return NULL if no map data) */
    
    /* Test cabin discovery */
    GList *cabins = poi_search_cabins(&rest_stop, 5.0);
    /* Function should execute (may return NULL if no map data) */
...
int main(void) {
    ...
    printf("Running Rest plugin integration tests (using actual Navit/plugin functions)...\n");
    
    failures += test_hiking_rest_intervals();
    failures += test_cycling_rest_intervals();
    ...
    failures += test_rest_stop_creation();
