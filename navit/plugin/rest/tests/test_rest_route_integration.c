...
static void init_navit_system(void) {
    ...
    atom_init();
    main_init("test_rest_route_integration");
    file_init();
...
static struct mapset *load_test_map_data(void) {
    ...
    map_file = g_build_filename("..", "..", "..", "navit", "plugin", "rest", "tests", "map_data", 
    ...
    if (!g_file_test(map_file, G_FILE_TEST_EXISTS)) {
        ...
        map_file = g_build_filename("/mnt/2e9a1e9f-2097-408c-ab9a-a01b32f11d28/github-projects/navit",
                                    "navit", "plugin", "rest", "tests", "map_data", 
                                    "osm_bbox_7.2,60.8,11.0,61.3.bin", NULL);
    }
    if (!g_file_test(map_file, G_FILE_TEST_EXISTS)) {
        g_free(map_file);
        ...
        map_file = g_build_filename(cwd, "..", "..", "..", "navit", "plugin", "rest", "tests", "map_data", 
                                    "osm_bbox_7.2,60.8,11.0,61.3.bin", NULL);
        g_free(cwd);
    }
...
static int test_hiking_rest_intervals_created(void) {
...
static int test_poi_discovery_hiking_route(void) {
    ...
    if (ms) {
        ...
        /* Use rest_poi_map_search_water for map-based water search */
        GList *water = rest_poi_map_search_water(&osm_node_bjordalsbu, 2.0, ms);
        if (water) {
            pois_found += g_list_length(water);
            printf("  Found %d water points near Bjordalsbu (map data)\n", g_list_length(water));
            rest_poi_free_list(water);
        }
...
static int test_poi_discovery_car_route(void) {
    ...
    if (ms) {
        printf("  Using map-based POI search (map data loaded)\n");
        /* Use map-based search if map data available */
        GList *pois = rest_poi_map_search_car_pois(&osm_node_moelv, 5.0, ms);
        if (pois) {
        ...
        GList *pois2 = rest_poi_map_search_car_pois(&osm_node_aukrust, 5.0, ms);
        if (pois2) {
            pois_found += g_list_length(pois2);
            printf("  Found %d POIs near Aukrustsenteret (map data)\n", g_list_length(pois2));
            rest_poi_free_list(pois2);
        }
...
static int test_cycling_rest_intervals_created(void) {
    /* Test with 100 km route */
    double route_distance = 100000.0;  /* 100 km */
    
    GList *stops = cycling_calculate_rest_stops(route_distance, 0);
...
int main(int argc, char **argv) {
    ...
    failures += test_hiking_rest_intervals_created();
    ...
    failures += test_cycling_rest_intervals_created();
