...
static void popup_show_visitbefore(struct navit *nav, struct pcoord *pc, void *menu) {
    ...
    if (dstcount >= 1) {
        ...
        for (i = 0; i < dstcount; i++) {
            ...
            popup_printf_cb(menuvisitbefore, menu_type_menu,
                            callback_new_3(callback_cast(popup_set_visitbefore), nav, pc, i), buffer);
        }
    }
}
        ...
        if (item) {
            ...
            if (item->type < type_line) {
                ...
                if (item_coord_get(item, &co, 1)) {
                    ...
                    if (diitem->map &&
                        map_get_attr(diitem->map, attr_type, &map_type, NULL) && map_type.u.str &&
                        !strcmp(map_type.u.str, "aprs")) {
...
void popup(struct navit *nav, int button, struct point *p) {
    ...
    c.x = co.x;
    c.y = co.y;
    popup_printf_cb(men, menu_type_menu, callback_new_2(callback_cast(popup_set_position), nav, &c),
                    _("Set as position"));
    ...
    popup_printf_cb(men, menu_type_menu, callback_new_2(callback_cast(popup_set_bookmark), nav, &c),
                    _("Add as bookmark"));
