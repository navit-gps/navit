...
int is_inside_lez(struct coord *coords, int number, struct coord point1, struct coord point2);
...
int is_inside_lez(struct coord *coords, int number, struct coord point1, struct coord point2) {
    ...
    struct graphics_gc *gc = graphics_gc_new(gr);
    ;
    ...
    int intersection2 = 0;
    int i;
    int maxdistw, maxdisth;
    float scalex, scaley, movetox, movetoy;
...
//calculate distance and scale to 1000 x 1000
    maxdistw = abs(max.x - min.x);
    maxdisth = abs(max.y - min.y);
    scalex = maxdistw / 700.0;
    scaley = maxdisth / 700.0;
    movetox = (0 - min.x);
    movetoy = (0 - min.y);
...
//        dp1[0].x = 0;
//        dp1[0].y = 0;
//        dp1[1].x = (point1.x + movetox) / scalex;
//        dp1[1].y = (point1.y + movetoy) / scaley;
//        dp2[0].x = 0; //(min.x-1)*0.01;
//        dp2[0].y = 0; //point2.y*0.01;
//        dp2[1].x = (point2.x + movetox) / scalex;
//        dp2[1].y = (point2.y + movetoy) / scaley;
        dlez[0].x = (coords[i].x + movetox) / scalex;
        dlez[0].y = (coords[i].y + movetoy) / scaley;
        dlez[1].x = (coords[i + 1].x + movetox) / scalex;
        dlez[1].y = (coords[i + 1].y + movetoy) / scaley;
        ...
        graphics_draw_mode(gr, draw_mode_begin);

        graphics_gc_set_foreground(gc, c);

        graphics_draw_lines(gr, gc, dlez, 2);
//        graphics_draw_lines(gr,gc,dp1, 2);
//        graphics_draw_lines(gr,gc,dp2, 2);

        graphics_draw_mode(gr, draw_mode_end);
    ...
    dbg(lvl_debug, "intersect start: %i intersect end: %i", (intersection1 & 1), (intersection2 & 1));
 ...
 * @param pos Coordinates to set as position
 */
void route_set_position(struct route *this, struct pcoord *pos, struct graphics *gra) {
    gr = gra;
...
void route_set_position_from_tracking(struct route *this, struct tracking *tracking, enum projection pro) {
    ...
    dbg(lvl_debug, "position 0x%x,0x%x item 0x%x,0x%x direction %d pos %d lenpos %d lenneg %d", c->x, c->y,
                sd ? sd->item.id_hi : 0, sd ? sd->item.id_lo : 0, ret->street_direction, ret->pos, ret->lenpos,
                ret->lenneg);
...
void route_graph_add_segment(struct route_graph *this, struct route_graph_point *start, struct route_graph_point *end,
    ...
    //check lez
    if (this->lezs) {
        for (int i = 0; i < this->lezs->nlezs; i++) {

            // Check if inside rect first to decrease process times
            if (((start->c.y >= this->lezs->lez[i]->min.y) && (start->c.x >= this->lezs->lez[i]->min.x)
                        && (start->c.x <= this->lezs->lez[i]->max.x) && (start->c.y <= this->lezs->lez[i]->max.y))
                        || ( (end->c.y >= this->lezs->lez[i]->min.y) && (end->c.x >= this->lezs->lez[i]->min.x)
                                    && (end->c.x <= this->lezs->lez[i]->max.x) && (end->c.y <= this->lezs->lez[i]->max.y)) ) {

                int inside = is_inside_lez(&this->lezs->lez[i]->coord[0], this->lezs->lez[i]->ncoords, start->c,
                            end->c);
...
//    }
    ...
    if (over->data.inside_lez) {
        dbg(lvl_error, "inside lez - start(%p): %i / %i, end(%llx): %i / %i", over->start, over->start->c.x,
...
static void route_graph_add_lez(struct route_graph *this, struct item *item, struct vehicleprofile *profile) {
    ...
    if (!strcmp(item_to_name(item->type), "poly_low_emission_zone")) {
        ...
        if (this->lezs->nlezs < 100) {
            this->lezs->lez[this->lezs->nlezs] = lez;
            this->lezs->nlezs++;
        } else {
            dbg(lvl_error, "To many lezs, skipping lez!");
        }

        for (i = 0; i < co_cnt; i++) {
            item_coord_get(item, &lez->coord[i], 1);
            if (lez->coord[i].x < lez->min.x)
                lez->min.x = lez->coord[i].x;
            if (lez->coord[i].y < lez->min.y)
                lez->min.y = lez->coord[i].y;
            if (lez->coord[i].x > lez->max.x)
                lez->max.x = lez->coord[i].x;
            if (lez->coord[i].y > lez->max.y)
                lez->max.y = lez->coord[i].y;

            dbg(lvl_debug, "coord[%i}: %i / %i", i, lez->coord[i].x, lez->coord[i].y);
        }

        dbg(lvl_error, "lez added: %i -- %i / %i, %i / %i", i, lez->min.x, lez->min.y, lez->max.x, lez->max.y);
    ...
    while (tmp1) {
        if (tmp1->start->c.x == prev->c.x && tmp1->start->c.y == prev->c.y
            ...
            dbg(lvl_debug, "found %s (0x%x,0x%x) (0x%x,0x%x)-(0x%x,0x%x) %p-%p", item_to_name(tmp1->data.item.type),
                        tmp1->data.item.id_hi, tmp1->data.item.id_lo, tmp1->start->c.x, tmp1->start->c.y,
                        tmp1->end->c.x, tmp1->end->c.y, tmp1->start, tmp1->end);
            while (tmp2) {
                dbg(lvl_debug, "compare %s (0x%x,0x%x) (0x%x,0x%x)-(0x%x,0x%x) %p-%p",
                            item_to_name(tmp2->data.item.type), tmp2->data.item.id_hi, tmp2->data.item.id_lo,
                            tmp2->start->c.x, tmp2->start->c.y, tmp2->end->c.x, tmp2->end->c.y, tmp2->start, tmp2->end);
...
street_get_data(struct item *item) {
    ...
    if (ret->flags & AF_CONDITIONAL_SPEED_LIMIT) {
        ...
        if ((ret->maxspeed_conditional_bwd = item_attr_get(item, attr_maxspeed_bwd_conditional_speed,
                    &maxspeed_bwd_cond_attr))) {
...
static int rm_attr_get(void *priv_data, enum attr_type attr_type, struct attr *attr) {
    ...
    case attr_label:
        ...
        if (mr->item.type == type_waypoint || mr->item.type == type_route_end) {
            ...
            mr->str = g_strdup_printf("%d",
                        route->reached_destinations_count + g_list_position(route->destinations, mr->dest) + 1);
            attr->u.str = mr->str;
            return 1;
        }
        return 0;
    ...
    case attr_debug:
        ...
        case type_rg_segment:
            ...
            mr->str = g_strdup_printf("len %d time %d start %p: %i / %i, end %p: %i / %i", seg->data.len,
                        route_time_seg(route->vehicleprofile, &seg->data, NULL, 0), seg->start, seg->start->c.x,
                        seg->start->c.y, seg->end, seg->end->c.x, seg->end->c.y);
            attr->u.str = mr->str;
            return 1;
            break;
        default:
...
static struct map_methods route_meth = { projection_mg, "utf-8", rm_destroy, rm_rect_new, rm_rect_destroy, rm_get_item,
        rm_get_item_byid,
        NULL,
        NULL,
        NULL, };

static struct map_methods route_graph_meth = { projection_mg, "utf-8", rp_destroy, rp_rect_new, rm_rect_destroy,
    ...
    case attr_vehicle:
        ...
        if (attr_updated) {
            ...
            if (vehicle_get_attr(this_->v, attr_position_coord_geo, &g, NULL)) {
                ...
                pc.x = c.x;
                pc.y = c.y;
                route_set_position(this_, &pc, gr);
            }
        }
        break;
    default:
...
struct object_func route_func = { attr_route, (object_func_new) route_new, (object_func_get_attr) route_get_attr,
        (object_func_iter_new) NULL, (object_func_iter_destroy) NULL, (object_func_set_attr) route_set_attr,
        (object_func_add_attr) route_add_attr, (object_func_remove_attr) route_remove_attr, (object_func_init) NULL,
        (object_func_destroy) route_destroy, (object_func_dup) route_dup, (object_func_ref) navit_object_ref,
