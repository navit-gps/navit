            ...
            while (argl) {
                ...
                if (speak_text) {
                        ...
                        for (index = speak_index_start; index <= speak_index_end; index++) {
                            ...
                            if (samplesmode == 1) {
                                ...
                                if (this->play_multiple_samples == 1) {
                                    ...
                                    if (file_exists(g_strdup(this->play[0]))) {
                                        ...
                                        dbg(lvl_error, "Can't execute '%s'. File does not exist.",
                                            g_strdup(this->play[0]));
                                    }
                                ...
                                // TTS
                                // Using new_arg because of 'free' command at the end of this function
                                new_arg = g_strdup_printf("%s", text);
                                this->play[this->play_text_argument_position] = new_arg;

                                // Execute cli speech command for TTS text
                                if (file_exists(g_strdup(this->play[0]))) {
                                    dbg(lvl_debug, "executing (TTS): %s", g_strdup(this->play[0]));
                                    dbg(lvl_debug, "text: %s", g_strdup(this->play[this->play_text_argument_position]));
                                    if (this->spi) {
                                        // Block until previous spawned speech process is terminated.
                                        spawn_process_check_status(this->spi, 1);
                                        spawn_process_info_free(this->spi);
                                    ...
                                    dbg(lvl_error, "Can't execute '%s'. File does not exist.", g_strdup(this->play[0]));
                                }
                            }  // samplesmode
                        }  // for sample index

                        // Play all samples using one command
                        if (samplesmode == 1 && this->play_multiple_samples == 1) {
                            // Execute cli speech command for all sample files
                            if (file_exists(g_strdup(play_multiple_samples[0]))) {
                                ...
                                dbg(lvl_error, "Can't execute '%s'. File does not exist.",
                                    g_strdup(play_multiple_samples[0]));
                            }
                        }
                    }  // if sample_count > 0
                    ...
                    if (samplesmode && missing) {
                        ...
                        // Execute cli speech command for TTS text
                        if (file_exists(g_strdup(g_strdup(this->play_tts[0])))) {
                            ...
                            dbg(lvl_error, "Can't execute '%s'. File does not exist.", g_strdup(this->play_tts[0]));
                        }
