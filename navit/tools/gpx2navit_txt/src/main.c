...
void usage(char **argv) {
    ...
    fprintf(stdout, "-o, --output            Sets output basename. The default is (source file\n");
...
void setDefault(g2sprop *prop) {
    ...
    if (prop->output == NULL) {
        ...
        prop->output = (char *)malloc(sizeof(char) * strlen(prop->sourcefile) + 1);
        if (0 == strcmp(dot, ".gpx")) {
...
void setOptions(int argc, char **argv, g2sprop *prop) {
    ...
    while ((result = getopt_long(argc, argv, "3wtrao:epfP:l:m:bS4:5:6:7:8:LTSsvg09", long_options, NULL)) != -1) {
        switch (result) {
        case '3': /* 3d output */
            prop->is3d = 1;
            break;
