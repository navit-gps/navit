...
void setPathData(pathattr *pattr, g2sattr *attr) {
    ...
    if ((pattr->count % reallocsize) == 0) {
        pattr->point = realloc(pattr->point, sizeof(double) * 3 * (pattr->count + reallocsize));
    }
...
int checkPath(parsedata *pdata) {
    ...
    if (pattr->count < prop->minpoints) {
        fprintf(stderr, "gpx2navit_txt:%s:%i track was not converted because of less then %d points. \n",
                prop->sourcefile, XML_GetCurrentLineNumber(pdata->parser), prop->minpoints);
        countUnconverted(pdata);
        return 0;
        /* check path length */
    } else if (pattr->length < prop->minlength * prop->length2meter) {
        fprintf(stderr, "gpx2navit_txt:%s:%i track was not converted because it is shorter than %dm.\n",
                prop->sourcefile, XML_GetCurrentLineNumber(pdata->parser), prop->minlength);
        countUnconverted(pdata);
        return 0;
        /* check path time */
    } else if (pattr->interval < prop->mintime * prop->time2sec) {
        fprintf(stderr, "gpx2navit_txt:%s:%i track was not converted because it is shorter than %d sed.\n",
                prop->sourcefile, XML_GetCurrentLineNumber(pdata->parser), prop->mintime);
        countUnconverted(pdata);
        return 0;
        /* check path speed */
    } else if (!prop->nospeedcheck && pattr->speed == .0) {
        fprintf(stderr,
                "gpx2navit_txt:%s:%i track was not converted because no move recorded. Use --no-speed-check option to "
                "bypass this check.\n",
...
void setPath(parsedata *pdata) {
    ...
    g2sprop *prop = pdata->prop;
    int isOk = 0;
    pattr->speed = getSpeed(pattr->length, pattr->interval, prop->speed2meter, prop->speed2sec);
    if (prop->isFast) {
        isOk = 1;
    } else {
    ...
    if (isOk) {
        ...
        fprintf(pdata->fp, "type=track label=\"%s\" desc=\"%s\" type=\"%s\"\ length=\"%5.3f\" count=\"%5d\"\n",
                pdata->pattr->name, pdata->pattr->desc, pdata->pattr->type, pdata->pattr->length, pdata->pattr->count);
