...
static int traffic_traff_http_init(struct traffic_priv *this_) {
     ...
     */
    const char *system = event_system();
    if (!system) {
        dbg(lvl_error, "event system not initialized, preventing the traff_http plugin from working");
        return 0;
    } else if (!strcmp("null", system) || !strcmp("opengl", system)) {
        /* null and opengl do not implement functions we require */
        dbg(lvl_error, "event system %s is incomplete, preventing the traff_http plugin from working", system);
        return 0;
    } else if (strcmp("glib", system) && strcmp("android", system)) {
        /*
         ...
         */
        dbg(lvl_error, "event system %s is not thread-safe and cannot be used with the traff_http plugin", system);
        return 0;
    }
