...
static void on_simple_ready(GObject *source_object, GAsyncResult *res, gpointer user_data) {
    ...
    g_signal_connect(priv->simple, "notify::location", G_CALLBACK(print_location), priv);
}
 ...
 */
static struct vehicle_priv *vehicle_geoclue_new(struct vehicle_methods *meth, struct callback_list *cbl,
                                                struct attr **attrs) {
    ...
    *meth = vehicle_geoclue_methods;
    ...
    ret->cbl = cbl;
    gclue_simple_new("navit", GCLUE_ACCURACY_LEVEL_EXACT, NULL, on_simple_ready, ret);
    return ret;
};
