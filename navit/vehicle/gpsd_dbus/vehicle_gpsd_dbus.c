...
static DBusHandlerResult vehicle_gpsd_dbus_filter(DBusConnection *connection, DBusMessage *message, void *user_data) {
    ...
    if (dbus_message_is_signal(message, "org.gpsd", "fix")) {
        dbus_message_get_args(message, NULL, DBUS_TYPE_DOUBLE, &time, DBUS_TYPE_INT32, &mode, DBUS_TYPE_DOUBLE, &ept,
                              DBUS_TYPE_DOUBLE, &latitude, DBUS_TYPE_DOUBLE, &longitude, DBUS_TYPE_DOUBLE, &eph,
                              DBUS_TYPE_DOUBLE, &altitude, DBUS_TYPE_DOUBLE, &epv, DBUS_TYPE_DOUBLE, &track,
                              DBUS_TYPE_DOUBLE, &epd, DBUS_TYPE_DOUBLE, &speed, DBUS_TYPE_DOUBLE, &eps,
                              DBUS_TYPE_DOUBLE, &climb, DBUS_TYPE_DOUBLE, &epc, DBUS_TYPE_STRING, &devname,
                              DBUS_TYPE_INVALID);
...
static int vehicle_gpsd_dbus_open(struct vehicle_priv *priv) {
    ...
    if (!priv->connection) {
        dbg(lvl_error, "Failed to open connection to %s message bus: %s", priv->address ? priv->address : "session",
            error.message);
...
static struct vehicle_priv *vehicle_gpsd_dbus_new(struct vehicle_methods *meth, struct callback_list *cbl,
                                                  struct attr **attrs) {
