...
/** @file voice.c
 * @defgroup voice-plugins voice plugins
 * @ingroup plugins
 * @brief Generic components of the voice object.
 *
 * This file implements the generic voice interface, i.e. everything which is
...
int voice_set_attr(struct vehicle *this_, struct attr *attr) {
    ...
    /* attr_profilename probably is never used by voice itself but it's used to control the
      routing engine. So any voice should allow to set and read it. */
 ...
 *
 * Used to remove a callback from the voice.
 * @param this_ A voice
 * @param attr
 */
...
void voice_set_cursor(struct vehicle *this_, struct cursor *cursor, int overwrite) {
    ...
    /* voice_draw will care for the graphics */
}

/**
 * Draws a voice on top of a graphics.
 *
 * @param this_ The voice
 * @param gra The graphics
 * @param pnt Screen coordinates of the voice.
...
static void voice_log_gpx(struct vehicle *this_, struct log *log) {
    ...
    // <geoidheight> Height (in meters) of geoid (mean sea level) above WGS84 earth ellipsoid. As defined in NMEA GGA message (field 11, which voice_wince.c ignores)
