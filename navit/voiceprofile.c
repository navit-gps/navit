...
static void voiceprofile_set_attr_do(struct voiceprofile *this_, struct attr *attr) {
    ...
    case attr_name:
        ...
        break;
/*
    case attr_voice_axle_weight:
        this_->axle_weight=attr->u.num;
        break;
    case attr_voice_dangerous_goods:
        this_->dangerous_goods=attr->u.num;
        break;
    case attr_voice_height:
        this_->height=attr->u.num;
        break;
    case attr_voice_length:
        this_->length=attr->u.num;
        break;
    case attr_voice_weight:
        this_->weight=attr->u.num;
        break;
    case attr_voice_width:
...
static void voiceprofile_free_hash_item(gpointer key, gpointer value, gpointer user_data) {
    struct navit_object *obj=value;
    obj->func->unref(obj);
}

static void voiceprofile_free_hash(struct voiceprofile *this_) {
    if (this_->roadprofile_hash) {
        g_hash_table_foreach(this_->roadprofile_hash, voiceprofile_free_hash_item, NULL);
        g_hash_table_destroy(this_->roadprofile_hash);
    }
}

static void voiceprofile_clear(struct voiceprofile *this_) {
    this_->mode=0;
...
*/
    voiceprofile_free_hash(this_);
//    this_->roadprofile_hash=g_hash_table_new(NULL, NULL);
}

static void voiceprofile_apply_roadprofile(struct voiceprofile *this_, struct navit_object *rp, int is_option) {
    ...
    if (rp->func->get_attr(rp, attr_item_types, &item_types_attr, NULL)) {
        ...
        while (*types != type_none) {
            ...
            if(!item_get_default_flags(*types))
                dbg(lvl_error,
                    "On '%s' roads used in '%s' voiceprofile access restrictions are ignored. You might even be directed to drive in wrong direction on a one-way road. "
...
static void voiceprofile_apply_attrs(struct voiceprofile *this_, struct navit_object *obj, int is_option) {
    struct attr attr;
    ...
    while (obj->func->get_attr(obj, attr_any, &attr, iter)) {
        ...
        if (attr.type == attr_roadprofile)
            voiceprofile_apply_roadprofile(this_, attr.u.navit_object, is_option);
        else if (attr.type != attr_profile_option)
            voiceprofile_set_attr_do(this_, &attr);
    }
    obj->func->iter_destroy(iter);
}

static void voiceprofile_debug_roadprofile(gpointer key, gpointer value, gpointer user_data) {
...
static void voiceprofile_update(struct voiceprofile *this_) {
    struct attr_iter *iter=voiceprofile_attr_iter_new(NULL);
    struct attr profile_option;
    dbg(lvl_debug,"enter");
    voiceprofile_clear(this_);
    voiceprofile_apply_attrs(this_, (struct navit_object *)this_, 0);
    while (voiceprofile_get_attr(this_, attr_profile_option, &profile_option, iter)) {
        ...
        if (active.u.num)
            voiceprofile_apply_attrs(this_, profile_option.u.navit_object, 1);
    }
    voiceprofile_attr_iter_destroy(iter);
...
*/
    g_hash_table_foreach(this_->roadprofile_hash, voiceprofile_debug_roadprofile, NULL);

}


struct voiceprofile *
voiceprofile_new(struct attr *parent, struct attr **attrs) {
    struct voiceprofile *this_;
    struct attr **attr, *type_attr;
    if (! (type_attr=attr_search(attrs, attr_name))) {
        return NULL;
    }
    this_=g_new0(struct voiceprofile, 1);
    this_->func=&voiceprofile_func;
    navit_object_ref((struct navit_object *)this_);
    this_->attrs=attr_list_dup(attrs);
    this_->active_callback.type=attr_callback;
    this_->active_callback.u.callback=callback_new_attr_1(callback_cast(voiceprofile_update), attr_active, this_);
    voiceprofile_clear(this_);
...
struct attr_iter *
voiceprofile_attr_iter_new(void* unused) {
...
int voiceprofile_add_attr(struct voiceprofile *this_, struct attr *attr) {
    ...
    case attr_roadprofile:
        voiceprofile_apply_roadprofile(this_, attr->u.navit_object, 0);
        break;
...
struct roadprofile *
voiceprofile_get_roadprofile(struct voiceprofile *this_, enum item_type type) {
...
static int voiceprofile_init(struct voiceprofile *this_) {
    voiceprofile_update(this_);
    return 0;
}

struct object_func voiceprofile_func = {
    attr_voiceprofile,
    (object_func_new)voiceprofile_new,
    (object_func_get_attr)voiceprofile_get_attr,
    (object_func_iter_new)voiceprofile_attr_iter_new,
    (object_func_iter_destroy)voiceprofile_attr_iter_destroy,
    (object_func_set_attr)voiceprofile_set_attr,
    (object_func_add_attr)voiceprofile_add_attr,
    (object_func_remove_attr)voiceprofile_remove_attr,
    (object_func_init)voiceprofile_init,
