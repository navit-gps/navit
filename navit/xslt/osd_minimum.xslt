...
<xsl:transform version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xi="http://www.w3.org/2001/XInclude">
   <xsl:template match="/config/navit/osd[1]">
      ...
      <xsl:text>&#x0A;        </xsl:text>
      <osd type="navigation_status" x="0" y="0" w="{$NEXT_TURN_SIZE}" h="{$NEXT_TURN_SIZE}" icon_src="%s_wh_{$ICON_BIG}_{$ICON_BIG}.png" enable_expression="navigation.nav_status==-1 || navigation.nav_status==1 || navigation.nav_status==2"/>
      <xsl:text>&#x0A;        </xsl:text>
      ...
      <xsl:text>&#x0A;        </xsl:text>
      <osd type="button" src="zoom_manual_{number($ICON_MEDIUM)}_{number($ICON_MEDIUM)}.png" command="autozoom_active=0" x="{round(8*number($OSD_SIZE))}" y="{-round(number($ICON_MEDIUM)+22*number($OSD_SIZE))}" osd_configuration="1" use_overlay="{$OSD_USE_OVERLAY}" enable_expression="autozoom_active!=0"/>
      <xsl:text>&#x0A;        </xsl:text>
      <osd type="button" src="zoom_auto_{number($ICON_MEDIUM)}_{number($ICON_MEDIUM)}.png" command="autozoom_active=1" x="{round(8*number($OSD_SIZE))}" y="{-round(number($ICON_MEDIUM)+22*number($OSD_SIZE))}" osd_configuration="1" use_overlay="{$OSD_USE_OVERLAY}" enable_expression="autozoom_active==0"/>
      <xsl:text>&#x0A;        </xsl:text>
      <osd type="button" src="zoom_in_{number($ICON_MEDIUM)}_{number($ICON_MEDIUM)}.png" command="zoom_in()" x="{round((number($ICON_MEDIUM)+8*number($OSD_SIZE))+8*number($OSD_SIZE))}" y="{-round(number($ICON_MEDIUM)+22*number($OSD_SIZE))}" osd_configuration="1" use_overlay="{$OSD_USE_OVERLAY}"/>
      <xsl:text>&#x0A;        </xsl:text>
