...
<xsl:stylesheet version="1.0"
 ...
 xmlns:xi="http://www.w3.org/2001/XInclude">
   ...
   <xsl:template match="/config/navit/graphics[1]">
      <graphics type="qt5" qt5_widget="qml_norotate" qt5_platform="ubuntumirclient" virtual_dpi="245"/>
      <xsl:text>&#x0A; 		</xsl:text>
      <xsl:copy>
   ...
   <xsl:template match="/config/navit/vehicle[1]">
		<vehicle name="Qt5" src="lomiri" profilename="car" enabled="yes" active="1" source="qt5://"/>
      <xsl:text>&#x0A; 		</xsl:text>
      <xsl:copy>
         <xsl:apply-templates select="@*"/>
         <xsl:apply-templates select="node()"/>
