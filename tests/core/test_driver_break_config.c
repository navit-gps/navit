 ...
 *
 * Unit tests for Driver Break plugin configuration and driving time
 * calculations
 */
...
static int test_config_defaults(void) {
  ...
  TEST_ASSERT(config.car_soft_limit_hours == 7,
              "Default car soft limit incorrect");
  ...
  TEST_ASSERT(config.truck_mandatory_break_after_hours == 4,
              "Default truck break hours incorrect");
  TEST_ASSERT(config.truck_mandatory_break_duration_min == 45,
              "Default truck break duration incorrect");
  TEST_ASSERT(config.min_distance_from_buildings == 150,
              "Default min distance incorrect");
...
static int test_driving_session_init(void) {
  ...
  TEST_ASSERT(session.total_driving_minutes == 0,
              "Total driving should start at 0");
  TEST_ASSERT(session.continuous_driving_minutes == 0,
              "Continuous driving should start at 0");
  TEST_ASSERT(session.mandatory_break_required == 0,
              "No mandatory break initially");
...
static int test_truck_mandatory_break_calculation(void) {
  ...
  int should_require_break =
      (driving_minutes >= (config.truck_mandatory_break_after_hours * 60));

  TEST_ASSERT(should_require_break == 1,
              "Truck should require break after 4.5 hours");

  /* Test: 3 hours = 180 minutes should NOT require mandatory break */
  driving_minutes = 180;
  should_require_break =
      (driving_minutes >= (config.truck_mandatory_break_after_hours * 60));
  TEST_ASSERT(should_require_break == 0,
              "Truck should NOT require break after 3 hours");
...
static int test_car_break_recommendation(void) {
  ...
  int should_recommend =
      (driving_minutes >= (config.car_break_interval_hours * 60));

  TEST_ASSERT(should_recommend == 1,
              "Car should recommend break after 4 hours");

  /* Test: 2 hours = 120 minutes should NOT trigger recommendation */
  driving_minutes = 120;
  should_recommend =
      (driving_minutes >= (config.car_break_interval_hours * 60));
  TEST_ASSERT(should_recommend == 0,
              "Car should NOT recommend break after 2 hours");
...
static int test_daily_driving_limits(void) {
  ...
  /* Car limits */
  TEST_ASSERT(config.car_soft_limit_hours >= 5 &&
                  config.car_soft_limit_hours <= 9,
              "Car soft limit should be 5-9 hours");
  TEST_ASSERT(config.car_max_hours >= 10 && config.car_max_hours <= 12,
              "Car max limit should be 10-12 hours");

  /* Truck limits (EU Regulation EC 561/2006) */
  TEST_ASSERT(config.truck_max_daily_hours == 9,
              "Truck max daily hours should be 9 (EU Regulation)");
  TEST_ASSERT(
      config.truck_mandatory_break_after_hours == 4,
...
static int test_driver_break_interval_range(void) {
  ...
  TEST_ASSERT(config.driver_break_interval_min_hours == 1,
              "Min rest interval should be 1 hour");
  TEST_ASSERT(config.driver_break_interval_max_hours == 6,
              "Max rest interval should be 6 hours");
  TEST_ASSERT(config.driver_break_interval_min_hours <
