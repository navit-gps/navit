...
static int test_db_get_history(void) {
  ...
  struct driver_break_stop_history *retrieved =
      (struct driver_break_stop_history *)history->data;
  ...
  GList *l = history;
  while (l) {
    struct driver_break_stop_history *h =
        (struct driver_break_stop_history *)l->data;
    g_free(h->name);
    g_free(h);
    l = g_list_next(l);
  }
...
static int test_db_clear_history(void) {
  ...
  TEST_ASSERT(g_list_length(history) == 1,
              "Wrong number of entries after clear");

  struct driver_break_stop_history *retrieved =
      (struct driver_break_stop_history *)history->data;
  TEST_ASSERT(strcmp(retrieved->name, "Recent Entry") == 0,
              "Wrong entry remaining");
...
static int test_db_save_load_config(void) {
  ...
  TEST_ASSERT(loaded_config.vehicle_type == DRIVER_BREAK_VEHICLE_TRUCK,
              "Vehicle type mismatch");
  TEST_ASSERT(loaded_config.car_soft_limit_hours == 7,
              "Car soft limit mismatch");
  TEST_ASSERT(loaded_config.truck_mandatory_break_after_hours == 4,
              "Truck break hours mismatch");
  TEST_ASSERT(loaded_config.min_distance_from_buildings == 150,
              "Min distance mismatch");

  /* Verify fuel configuration persisted correctly */
  TEST_ASSERT(loaded_config.fuel_type == DRIVER_BREAK_FUEL_DIESEL,
              "Fuel type mismatch");
  TEST_ASSERT(loaded_config.fuel_tank_capacity_l == 70,
              "Fuel tank capacity mismatch");
  TEST_ASSERT(loaded_config.fuel_avg_consumption_x10 == 85,
              "Fuel avg consumption mismatch");
  TEST_ASSERT(loaded_config.fuel_obd_available == 1, "Fuel OBD flag mismatch");
  TEST_ASSERT(loaded_config.fuel_j1939_available == 0,
              "Fuel J1939 flag mismatch");
  TEST_ASSERT(loaded_config.fuel_ethanol_manual_pct == 10,
              "Fuel ethanol manual pct mismatch");
  TEST_ASSERT(loaded_config.fuel_low_warning_km == 100,
              "Fuel low warning km mismatch");
  TEST_ASSERT(loaded_config.fuel_search_buffer_km == 25,
              "Fuel search buffer km mismatch");
  TEST_ASSERT(loaded_config.fuel_high_load_threshold == 30,
              "Fuel high load threshold mismatch");
 ...
 * verify that driver_break_db_clean_corrupted_config()/load_config discards
 * them and falls back to defaults rather than propagating invalid values. */
static int test_db_malformed_config_entries(void) {
  ...
  TEST_ASSERT(
      result == 0,
      "Malformed-only config should not be reported as successfully loaded");
  TEST_ASSERT(loaded_config.car_soft_limit_hours == 0,
              "Malformed car_soft_limit_hours should be ignored");
  TEST_ASSERT(loaded_config.truck_max_daily_hours == 0,
              "Malformed truck_max_daily_hours should be ignored");
  TEST_ASSERT(loaded_config.fuel_avg_consumption_x10 == 0,
              "Malformed fuel_avg_consumption_x10 should be ignored");

  driver_break_db_destroy(db);
  unlink(db_path);
  g_free(db_path);
  (void)result; /* result may be 0 if all entries were skipped; this is
                   acceptable. */
  return 0;
}
...
static int test_db_add_fuel_stop(void) {
  ...
  rc = sqlite3_prepare_v2(raw,
                          "SELECT COUNT(*), SUM(fuel_added), "
                          "SUM(fuel_level_after), MAX(ethanol_pct) "
...
static int test_db_add_fuel_sample_and_trip_summary(void) {
  ...
  int result =
      driver_break_db_add_fuel_sample(db, now - 60, 500.0, 0.4, 8.0, 60.0, 50);
  ...
  /* Add a trip summary */
  result = driver_break_db_add_trip_summary(db, now - 600, now, 20000.0, 12.0,
                                            7.0, 10.5, 1);
  ...
  TEST_ASSERT(sum_distance > 1000.0 && sum_distance < 2000.0,
              "Fuel samples distance sum unexpected");
  TEST_ASSERT(sum_fuel > 0.8 && sum_fuel < 1.0,
              "Fuel samples fuel sum unexpected");

  sqlite3_finalize(stmt);

  /* Check trip summaries row count */
  rc = sqlite3_prepare_v2(raw,
                          "SELECT COUNT(*), SUM(total_distance_m), "
                          "SUM(total_fuel), MAX(high_load_flag) "
