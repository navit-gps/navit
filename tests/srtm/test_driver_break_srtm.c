...
static size_t curl_write_file_cb(void *ptr, size_t size, size_t nmemb,
                                 void *user) {
  return fwrite(ptr, size, nmemb, (FILE *)user);
}

/* Browser User-Agent required by Viewfinder (blocks scripted downloads without
 * it). */
...
static int create_valid_hgt_file(const char *filepath, int lon_idx,
                                 int lat_idx) {
...
static int test_hgt_valid_file(void) {
  ...
  TEST_ASSERT(create_valid_hgt_file(filepath, 7, 60) == 1,
              "Failed to create valid HGT file");
  ...
  TEST_ASSERT(elevation >= -500 && elevation <= 9000,
...
static int test_hgt_corrupt_size(void) {
  ...
  TEST_ASSERT(create_corrupt_hgt_file_size(filepath) == 1,
              "Failed to create corrupt HGT file");
  ...
  TEST_ASSERT(elevation == -32768 || elevation == 0,
              "Corrupt file not handled correctly");
...
static int test_hgt_corrupt_data(void) {
  ...
  TEST_ASSERT(create_corrupt_hgt_file_data(filepath) == 1,
              "Failed to create corrupt HGT file");
  ...
  int elevation = srtm_get_elevation(&coord);
  /* Should return void value or handle gracefully */
  TEST_ASSERT(elevation == -32768 || elevation >= -32768,
              "Invalid data not handled correctly");
...
static int test_hgt_corrupt_truncated(void) {
  ...
  TEST_ASSERT(create_corrupt_hgt_file_truncated(filepath) == 1,
              "Failed to create truncated HGT file");
  ...
  /* Should return void value or handle gracefully */
  TEST_ASSERT(elevation == -32768 || elevation == 0,
              "Truncated file not handled correctly");
...
static int test_hgt_tile_filename(void) {
  ...
  TEST_ASSERT(strcmp(filename, "N60E007.hgt") == 0,
              "Tile filename mismatch for positive coords");
  g_free(filename);

  /* Test negative coordinates */
  filename = srtm_get_tile_filename(-7, -60);
  TEST_ASSERT(filename != NULL, "Tile filename is NULL");
  TEST_ASSERT(strcmp(filename, "S60W007.hgt") == 0,
              "Tile filename mismatch for negative coords");
  g_free(filename);

  /* Test zero coordinates */
  filename = srtm_get_tile_filename(0, 0);
  TEST_ASSERT(filename != NULL, "Tile filename is NULL");
  TEST_ASSERT(strcmp(filename, "N00E000.hgt") == 0,
              "Tile filename mismatch for zero coords");
  g_free(filename);
...
static int create_constant_hgt_file(const char *filepath,
                                    int16_t elevation_value) {
  FILE *f = fopen(filepath, "wb");
  if (!f)
    return 0;
  const int size = 3601 * 3601;
...
/* Test that elevation lookup at tile borders uses the correct tile per
 * coordinate. Each point is resolved to one tile via floor(lon), floor(lat); no
...
static int test_hgt_tile_borders(void) {
  ...
  /* Two adjacent tiles with distinct constant elevations so we can verify which
   * tile is read */
...
/* Download one SRTM HGT tile (zip from Viewfinder dem3 or NASA), extract .hgt
 * into test_dir. Returns 1 on success. */
static int download_hgt_tile(int lon, int lat, const char *test_dir) {
  ...
  int lon_abs = abs(lon);
  char *filename =
      g_strdup_printf("%c%02d%c%03d.hgt", lat_dir, lat_abs, lon_dir, lon_abs);
  char *tilename_zip =
      g_strdup_printf("%c%02d%c%03d.zip", lat_dir, lat_abs, lon_dir, lon_abs);
  char *zip_path = g_build_filename(test_dir, tilename_zip, NULL);
  const char *zone = viewfinder_zone(lat, lon);
  char *url_vf = zone ? g_strdup_printf("%s%s%s", SRTM_URL_VIEWFINDER_DEM3,
                                        zone, tilename_zip)
                      : NULL;
  char *url_nasa =
      g_strdup_printf("%s%c%02d%c%03d.SRTMGL1.hgt.zip", SRTM_URL_NASA, lat_dir,
  ...
  char *extract_cmd =
      g_strdup_printf("cd '%s' && unzip -o -q '%s' '%s' 2>/dev/null", test_dir,
                      zip_path, filename);
  ...
  if (unzip_ret != 0 || !g_file_test(hgt_path, G_FILE_TEST_EXISTS)) {
    /* NASA zips often contain NxxEyy.SRTMGL1.hgt instead of NxxEyy.hgt; extract
     * and rename */
    char *nasa_name = g_strdup_printf("%c%02d%c%03d.SRTMGL1.hgt", lat_dir,
                                      lat_abs, lon_dir, lon_abs);
    extract_cmd =
        g_strdup_printf("cd '%s' && unzip -o -q '%s' '%s' 2>/dev/null",
                        test_dir, zip_path, nasa_name);
...
/* Download SRTM HGT tiles for the same three OSM locations as the GeoTIFF test;
 * verify read. */
static int test_srtm_hgt_download_and_read(void) {
  ...
  /* Tiles for (62.0937,7.1433), (61.5919,9.7018), (61.36012,9.66941): N62E007,
   * N61E009 */
  int got_62_7 = download_hgt_tile(7, 62, test_dir);
  int got_61_9 = download_hgt_tile(9, 61, test_dir);

  if (!got_62_7 || !got_61_9) {
    printf("SRTM HGT: tile download failed or unzip failed "
           "(network/unavailable). Skip read test.\n");
    g_free(test_dir);
    return 0;
  }
  ...
  TEST_ASSERT(e1 >= -50 && e1 <= 3000,
              "SRTM HGT elevation at point1 out of expected range");
  TEST_ASSERT(e2 != -32768, "SRTM HGT read at 61.5919,9.7018 should be valid");
  TEST_ASSERT(e2 >= -50 && e2 <= 3000,
              "SRTM HGT elevation at point2 out of expected range");
  TEST_ASSERT(e3 != -32768,
              "SRTM HGT read at 61.36012,9.66941 should be valid");
  TEST_ASSERT(e3 >= -50 && e3 <= 3000,
              "SRTM HGT elevation at point3 out of expected range");

  printf("SRTM HGT: tiles downloaded and read correctly at 3 OSM locations "
         "(62.09,7.14 / 61.59,9.70 / 61.36,9.67).\n");
  g_free(test_dir);
  return 0;
}
#endif
...
#ifdef HAVE_GEOTIFF
/* Check file has TIFF magic (II or MM in first 2 bytes). Returns 1 if valid
 * TIFF, 0 otherwise. */
static int file_is_valid_tiff(const char *filepath) {
  ...
  if (n < 2)
    return 0;
  /* Little-endian TIFF: 0x49 0x49 (II); big-endian: 0x4D 0x4D (MM). Reject
   * HTML/XML (e.g. 0x3C 0x3F = "<?"). */
...
#ifdef HAVE_CURL
/* Download Copernicus GeoTIFF; if response is not a valid TIFF (e.g. S3
 * XML/HTML error), treat as failure. */
...
/* Test Copernicus GLO-30: download tiles for three OSM map locations (Norway)
 * and verify read. Locations: map=12/62.0937/7.1433, map=12/61.5919/9.7018,
 * map=15/61.36012/9.66941 Tiles: N62E007, N61E009. */
static int test_copernicus_glo30_download_and_read(void) {
  char *test_dir = g_strdup("/tmp/test_copernicus_glo30");
  g_mkdir_with_parents(test_dir, 0755);

#ifdef HAVE_CURL
  /* Download tiles from public AWS S3; tiles are in subdirs:
   * {base}/{tilename}/{tilename}.tif */
  ...
  char *path_62_7 = g_build_filename(
      test_dir, "Copernicus_DSM_COG_10_N62_00_E007_00_DEM.tif", NULL);
  char *path_61_9 = g_build_filename(
      test_dir, "Copernicus_DSM_COG_10_N61_00_E009_00_DEM.tif", NULL);
  ...
  if (!got_62_7 || !got_61_9) {
    printf("Copernicus GLO-30: tile download failed or skipped "
           "(network/unavailable). Skip read test.\n");
    g_free(test_dir);
    return 0;
  }
#else
  /* Without CURL we only run if tiles are already present and valid (e.g. from
   * a previous run) */
  char *path_62_7 = g_build_filename(
      test_dir, "Copernicus_DSM_COG_10_N62_00_E007_00_DEM.tif", NULL);
  char *path_61_9 = g_build_filename(
  ...
  int got_62_7 = g_file_test(path_62_7, G_FILE_TEST_EXISTS) &&
                 file_is_valid_tiff(path_62_7);
  int got_61_9 = g_file_test(path_61_9, G_FILE_TEST_EXISTS) &&
                 file_is_valid_tiff(path_61_9);
  g_free(path_62_7);
  g_free(path_61_9);
  if (!got_62_7 || !got_61_9) {
    printf("Copernicus GLO-30: tiles not present or invalid (no CURL). Skip "
           "test.\n");
    g_free(test_dir);
    return 0;
  }
  ...
  TEST_ASSERT(e1 != -32768,
              "Copernicus read at 62.0937,7.1433 should be valid");
  TEST_ASSERT(e1 >= -50 && e1 <= 3000,
              "Copernicus elevation at point1 out of expected range");

  TEST_ASSERT(e2 != -32768,
              "Copernicus read at 61.5919,9.7018 should be valid");
  TEST_ASSERT(e2 >= -50 && e2 <= 3000,
              "Copernicus elevation at point2 out of expected range");

  TEST_ASSERT(e3 != -32768,
              "Copernicus read at 61.36012,9.66941 should be valid");
  TEST_ASSERT(e3 >= -50 && e3 <= 3000,
              "Copernicus elevation at point3 out of expected range");

  printf("Copernicus GLO-30: tiles read correctly at 3 OSM locations "
         "(62.09,7.14 / 61.59,9.70 / 61.36,9.67).\n");
  g_free(test_dir);
  return 0;
}
